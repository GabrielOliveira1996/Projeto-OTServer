...
function initializeSpelllist()
    ...
    for _, spellName in ipairs(spells) do
        ...
        if info then
            ...
            -- Volta a usar o arquivo único (Atlas) e o recorte (Clip)
...
function updateSpelllist()
    -- Usar sempre 'Default' já que é o único profile com dados
    local currentProfile = 'Default'
    
    for spellName, info in pairs(SpellInfo[currentProfile]) do
        local tmpLabel = spellList:getChildById(spellName)
...
function updateSpellInformation(widget)
    ...
    if SpellInfo[currentProfile][spell] then
        local info = SpellInfo[currentProfile][spell]
...
function resizeWindow()
    ...
    spelllistWindow:setWidth(SpelllistSettings['Default'].spellWindowWidth +
                                 SpelllistSettings[currentProfile].iconSize.width - 32)
    spellList:setWidth(
        SpelllistSettings['Default'].spellListWidth + SpelllistSettings[currentProfile].iconSize.width - 32)
end
...
local opcodeCooldown = 85

ProtocolGame.registerExtendedOpcode(opcodeCooldown, function(protocol, opcode, buffer)
    -- O buffer chega como "Nome da Magia|Tempo"
    local params = buffer:split("|")
    local name = params[1]
    local duration = tonumber(params[2])

    if name and duration then
        -- 1. Faz o ícone na ActionBar rodar o visual
        if modules.game_actionbar then
            local actionButtons = modules.game_actionbar.getActionButtons()
            for _, button in pairs(actionButtons) do
                -- Verifica se o botão tem a magia com esse nome
                if button:getSpell() and button:getSpell().name == name then
                    button:setCooldown(duration, duration)
                end
            end
        end

        -- 2. Se a lista de magias estiver aberta, atualiza lá também
        -- (Opcional, dependendo da sua versão de spelllist)
    end
end)
