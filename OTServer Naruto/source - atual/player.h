...
public:
	...
	bool isKiba() const;
	uint32_t akamaruLevel;
	uint64_t akamaruExp;
	uint32_t akamaruPoints;
	uint32_t akamaruAttack;
	uint32_t akamaruAgility;
	uint32_t akamaruDodge;
	uint32_t akamaruHealthPts;
	uint32_t akamaruSpeedPts;
	uint32_t akamaruCurrentHp;
	uint32_t akamaruOutfit;
	bool akamaruIsDead;
	uint32_t akamaruDeathTimestamp;
	void addAkamaruExperience(uint64_t exp);

	uint64_t getAkamaruLevelExp(uint32_t level) const;
	uint64_t getAkamaruRequiredExpToNextLevel() const;

	uint32_t getAkamaruMaxHealth() const;
	uint32_t getAkamaruMaxSpeed() const;
	void addAkamaruPoint(const std::string &attr);
	void sendAkamaruData();
	// controle de vida do akamaru
	bool isAkamaruDead() const { return akamaruIsDead; }
	void setAkamaruDead(bool dead) { akamaruIsDead = dead; }
	uint32_t getAkamaruDeathTimestamp() const { return akamaruDeathTimestamp; }
	void setAkamaruDeathTimestamp(uint32_t timestamp) { akamaruDeathTimestamp = timestamp; }
	void updateAkamaruStatus(uint32_t currentHp, bool dead);

	uint32_t attributePoints;
	uint32_t getAttributePoints() const { return attributePoints; };
	void setAttributePoints(uint32_t n) { attributePoints = n; };
	void addAttributePoints(int32_t n)
	{
		attributePoints = (uint32_t)std::max((int32_t)0, (int32_t)(attributePoints + n));
	};
