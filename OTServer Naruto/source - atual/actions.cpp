...
bool Actions::registerEvent(Event *event, xmlNodePtr p, bool override)
{
	...
	if (readXMLString(p, "fromid", strValue) && readXMLString(p, "toid", endStrValue))
	{
		...
		if (intVector[0] && endIntVector[0] && intVector.size() == endIntVector.size())
		{
			...
			std::cout << "[Warning - Actions::registerEvent] Malformed entry (from item: \"" << strValue << "\", to item: \"" << endStrValue << "\")" << std::endl;
	}
	...
	if (readXMLString(p, "fromuid", strValue) && readXMLString(p, "touid", endStrValue))
	{
		...
		if (intVector[0] && endIntVector[0] && intVector.size() == endIntVector.size())
		{
			...
			std::cout << "[Warning - Actions::registerEvent] Malformed entry (from unique: \"" << strValue << "\", to unique: \"" << endStrValue << "\")" << std::endl;
	}
	...
	if (readXMLString(p, "fromaid", strValue) && readXMLString(p, "toaid", endStrValue))
	{
		...
		if (intVector[0] && endIntVector[0] && intVector.size() == endIntVector.size())
		{
			...
			std::cout << "[Warning - Actions::registerEvent] Malformed entry (from action: \"" << strValue << "\", to action: \"" << endStrValue << "\")" << std::endl;
	}
...
bool Actions::useItem(Player *player, const Position &pos, uint8_t index, Item *item)
{
	...
	// Get action first to check allowMovement flag
	Action *action = NULL;
	if ((action = getAction(item, ACTION_UNIQUEID)) ||
	...
	{
		// Only stop walking if allowMovement is false (default behavior)
		if (!action->getAllowMovement())
			player->stopWalk();
	}
	else
	{
		// No action found, use default behavior (stop walking)
		player->stopWalk();
	}
						...
						uint8_t toStackPos, Item *item, bool isHotkey, uint32_t creatureId /* = 0*/)
{
	...
	// 1. Busca a configuração da Action (onde está o seu allowMovement)
	...
	// 2. Lógica de movimento: se allowMovement for true no XML, ele NÃO para de andar
	if (!action->getAllowMovement())
	{
		player->stopWalk();
	}

	// 3. Define o delay para a próxima ação
	player->setNextAction(OTSYS_TIME() + g_config.getNumber(ConfigManager::EX_ACTIONS_DELAY_INTERVAL) - SCHEDULER_MINTICKS);

	// 4. [O BLOCO QUE ESTAVA SOLTO] - Prepara as posições e executa o item
	...
	// 5. Aqui o servidor finalmente chama o script .lua ou a ação interna
...
Action::Action(const Action *copy) : Event(copy)
{
	...
	allowMovement = copy->allowMovement;
}
...
bool Action::configureEvent(xmlNodePtr p)
{
	...
	if (readXMLString(p, "allowmovement", strValue) || readXMLString(p, "allowMovement", strValue))
		setAllowMovement(booleanString(strValue));
