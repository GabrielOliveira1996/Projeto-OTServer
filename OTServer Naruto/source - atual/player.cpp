...
void Player::addAkamaruExperience(uint64_t exp)
{
    ...
    // faz o loop de level up baseado na funï¿½ï¿½o oficial de xp do servidor
    while (akamaruExp >= getExpForLevel(akamaruLevel + 1)) {
        akamaruLevel++;
        akamaruPoints += 3; // pontos para o jogador distribuir depois

        // mensagem de progressao
        char buf[128];
        sprintf(buf, "Seu Akamaru subiu para o level %d!", akamaruLevel);
        sendTextMessage(MSG_EVENT_ADVANCE, buf);
        
        // efeito visual no pe do jogador
        g_game.addMagicEffect(getPosition(), (MagicEffect_t)10);
    }
}

// retorna o hp total do akamaru onde a cada ponto adicionado o mesmo recebe +10
uint32_t Player::getAkamaruMaxHealth() const {
    uint32_t baseHp = 150; // vida base
    return baseHp + (akamaruHealthPts * 10); // resultado final
}

// retorna a velocidade total do akamaru, onde cada ponto adicionado recebe +5
uint32_t Player::getAkamaruMaxSpeed() const {
    uint32_t baseSpeed = 200; // valor base de velocidade
    return baseSpeed + (akamaruSpeedPts * 5);
}

// retorna experiencia total para determinado nivel
uint64_t Player::getAkamaruLevelExp(uint32_t level) const {
    if (level <= 1) return 0;
    // formula oficial do player: (50/3) * (L^3 - 6L^2 + 17L - 12)
    return (uint64_t)((50ULL * level * level * level - 150ULL * level * level + 400ULL * level) / 3ULL) - 100ULL;
}

// retorna quanto de exp o akamaru precisa para passar do nível atual para o próximo
uint64_t Player::getAkamaruRequiredExpToNextLevel() const {
    uint32_t nextLevel = akamaruLevel + 1;
    uint64_t expForNext = getAkamaruLevelExp(nextLevel);
    uint64_t expForCurrent = getAkamaruLevelExp(akamaruLevel);
...
void Player::addExperience(uint64_t exp)
{
    ...
    if (exp > 0) {
        ...
        for(std::list<Creature*>::iterator it = summons.begin(); it != summons.end(); ++it) {
            // verifica se existe um summon com esse nome
        ...
        // Only add experience if the pet is summoned
