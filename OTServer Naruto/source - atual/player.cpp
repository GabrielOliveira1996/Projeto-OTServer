...
Player::Player(const std::string &_name, ProtocolGame *p) : Creature(), transferContainer(ITEM_LOCKER), name(_name), nameDescription(_name), client(p)
{
	...
	// akamaru
	akamaruLevel = 1;
	akamaruExp = akamaruPoints = akamaruHealthPts = akamaruSpeedPts = 0;
	akamaruAttack = akamaruAgility = akamaruDodge = 1;
	akamaruCurrentHp = 100;
	akamaruIsDead = false;
	akamaruDeathTimestamp = 0;
	akamaruOutfit = 0;
...
bool Player::isKiba() const
{
	for (size_t i = 0; i < VocationConstants::KIBA_VOCATIONS_SIZE; ++i)
	{
		if (getVocationId() == VocationConstants::KIBA_VOCATIONS[i])
		{
			return true;
		}
	}
...
int32_t Player::getDefense() const
{
	...
	// captura o skill shielding como base, com ou sem escudo.
	defenseSkill = std::max((int32_t)1, (int32_t)getSkill(SKILL_SHIELD, SKILL_LEVEL));
	if (shield)
	{
	...
	// 3. Verificaï¿½ï¿½o de seguranï¿½a para a Vocaï¿½ï¿½o (Evita o Crash)
	if (vocation)
	{
		double multiplier = vocation->getMultiplier(MULTIPLIER_DEFENSE);
		if (multiplier != 1.0 && multiplier > 0)
		{
			defenseValue = (int32_t)(defenseValue * multiplier);
		}
	}
	// 4. Cï¿½lculo final com cast explï¿½cito para evitar erros de precisï¿½o
	float result = ((float)(defenseSkill * 0.5f) + (defenseValue * 1.2f)) * defenseFactor;
	return (int32_t)std::ceil(result);
}
...
Depot *Player::getDepot(uint32_t depotId, bool autoCreateDepot)
{
	...
	if (autoCreateDepot)
	{
		...
		std::cout << "Failure: Creating a new depot with id: " << depotId << ", for player: " << getName() << std::endl;
	}
...
void Player::sendCancelMessage(ReturnValue message) const
{
	...
	case RET_NOTENOUGHLEVEL:
		sendCancel("Your level is too low.");
		break;

	case RET_NOTENOUGHMAGICLEVEL:
		sendCancel("Your Ninjutsu is too low.");
		break;
	...
	case RET_YOUAREEXHAUSTED:
		sendCancel("Wait, you are exhausted !");
		break;
	...
	case RET_YOUNEEDTOLEARNTHISSPELL:
		sendCancel("You need to learn this jutsu.");
		break;

	case RET_YOURVOCATIONCANNOTUSETHISSPELL:
		sendCancel("Your vocation cannot use this jutsu.");
		break;

	case RET_YOUNEEDAWEAPONTOUSETHISSPELL:
		sendCancel("You need to equip a weapon to use this jutsu.");
		break;
	...
	case RET_YOUNEEDAMAGICITEMTOCASTSPELL:
		sendCancel("You need a magic item to cast this jutsu.");
		break;
	...
	case RET_YOUMAYNOTCASTAREAONBLACKSKULL:
		sendCancel("You may not cast area spells while you have a aureole.");
		break;
...
void Player::sendExtendedOpcode(uint8_t opcode, const std::string &buffer)
{
	if (client)
	{
...
void Player::onCreatureDisappear(const Creature *creature, bool isLogout)
{
	...
	if (isLogout)
	{
		// antes de salvar, buscamos o hp atual do akamaru nos summons
		if (!summons.empty())
		{
			for (std::list<Creature *>::iterator it = summons.begin(); it != summons.end(); ++it)
			{
				std::string akamaruValue = "";
				if ((*it)->getStorage(STORAGE_AKAMARU_IDENTIFIER, akamaruValue) && atoi(akamaruValue.c_str()) == 1)
				{
					this->akamaruCurrentHp = (uint32_t)(*it)->getHealth();
					break;
				}
			}
		}
		// --- FIM DO BLOCO DO AKAMARU ---
...
{
	if (!hasFlag(PlayerFlag_CannotBeMuted) && g_config.getNumber(ConfigManager::MAX_MESSAGEBUFFER) != 0 && messageBuffer > 0)
		messageBuffer--;
}
...
void Player::drainMana(Creature *attacker, CombatType_t combatType, int32_t damage)
{
	...
	char buffer[150];
	if (attacker)
		sprintf(buffer, "You lose %d chakra blocking an attack by %s.", damage, attacker->getNameDescription().c_str());
	else
		sprintf(buffer, "You lose %d chakra.", damage);
...
void Player::addManaSpent(uint64_t amount, bool useMultiplier /* = true*/)
{
	...
	while (manaSpent + amount >= nextReqMana)
	{
		...
		char advMsg[50];
		sprintf(advMsg, "Your ninjutsu has been increased to %d.", magLevel);
...
void Player::addAkamaruExperience(uint64_t exp)
{
	if (exp <= 0)
		return;
	...
	while (akamaruExp >= getExpForLevel(akamaruLevel + 1))
	{
		akamaruLevel++;

		// verifica se o novo nível é múltiplo de 10
		if (akamaruLevel % 10 == 0)
		{
			akamaruPoints += 3; // Ganha 3 pontos em níveis como 10, 20, 30...
		}
		else
		{
			akamaruPoints += 1; // Ganha 1 ponto nos demais níveis
		}

		char buf[128];
		sprintf(buf, "Seu Akamaru subiu para o level %d!", akamaruLevel);
		sendTextMessage(MSG_EVENT_ADVANCE, buf);

		g_game.addMagicEffect(getPosition(), (MagicEffect_t)10);
	}

	if (client)
	{
		this->sendAkamaruData();
	}
}

// return current akamaru health
uint32_t Player::getAkamaruMaxHealth() const
{
	uint32_t baseHealth = 100;
	uint32_t healthFromPoints = akamaruHealthPts * 20;
	return baseHealth + healthFromPoints;
}

// return current akamaru speed
uint32_t Player::getAkamaruMaxSpeed() const
{
	uint32_t baseSpeed = 200; // base value for akamaru speed
	return baseSpeed + (akamaruSpeedPts * 5);
}

// return total experience for a given level
uint64_t Player::getAkamaruLevelExp(uint32_t level) const
{
	if (level <= 1)
		return 0;
	// official formula: (50/3) * (L^3 - 6L^2 + 17L - 12)
	return (uint64_t)((50ULL * level * level * level - 150ULL * level * level + 400ULL * level) / 3ULL) - 100ULL;
}

// return how much experience the akamaru needs to pass from the current level to the next
uint64_t Player::getAkamaruRequiredExpToNextLevel() const
{
	// get the total experience required for the next level, official formula
	uint64_t expForNext = getExpForLevel(akamaruLevel + 1);

	// get the total experience required for the current level, officinal formula
	uint64_t expForCurrent = getExpForLevel(akamaruLevel);

	// return diference between next and cuc
	return expForNext - expForCurrent;
}

void Player::updateAkamaruStatus(uint32_t currentHp, bool dead)
{
	this->akamaruCurrentHp = currentHp;
	this->akamaruIsDead = dead;

	if (dead)
	{
		this->akamaruDeathTimestamp = (uint32_t)time(NULL);
	}
	else
	{
		this->akamaruDeathTimestamp = 0;
	}
	if (client)
	{
		this->sendAkamaruData();
...
void Player::addAkamaruPoint(const std::string &attr)
{
	// verify if player has points
	if (akamaruPoints == 0)
	{
		sendTextMessage(MSG_STATUS_SMALL, "Você não possui pontos Akamaru disponíveis.");
		return;
	}
	...
	// message to player and update akamaru data
	if (success)
	{
		akamaruPoints--;
		sendTextMessage(MSG_STATUS_SMALL, "Ponto adicionado com sucesso!");
		this->sendAkamaruData();
	}
	else if (limitReached)
	{
		sendTextMessage(MSG_STATUS_SMALL, "Este atributo já atingiu o nível máximo (100).");
	}
}

void Player::sendAkamaruData()
{
	if (!client)
	{
		return;
	...
	OutputMessage_ptr msg = client->getOutputBuffer();
	if (!msg)
	{
		return;
	}
	...
	// byte sent to client
	msg->AddByte(0x99);
	msg->AddU32(akamaruLevel);

	uint64_t expForCurrentLevel = getExpForLevel(akamaruLevel);
	uint32_t relativeExp = 0;
	if (akamaruExp >= expForCurrentLevel)
	{
		relativeExp = (uint32_t)(akamaruExp - expForCurrentLevel);
	}

	msg->AddU32(relativeExp);
	msg->AddU32((uint32_t)getAkamaruRequiredExpToNextLevel());
	msg->AddU32(akamaruPoints);
	msg->AddU32(akamaruAttack);
	msg->AddU32(akamaruAgility);
	msg->AddU32(akamaruDodge);
	msg->AddU32(akamaruHealthPts);
	msg->AddU32(getAkamaruMaxHealth());
	msg->AddU32(akamaruSpeedPts);
	msg->AddU32(getAkamaruMaxSpeed());
	msg->AddU32(akamaruOutfit);
}
...
void Player::addExperience(uint64_t exp)
{
	...
	// --- LÓGICA DE DIVISÃO DE XP (DADOS) ---
	if (vocation->getId() >= 33 && vocation->getId() <= 37)
	{
		bool isAkamaruSummoned = false;
		std::list<Creature *> summons = getSummons();
		...
		if (isAkamaruSummoned)
		{
			uint64_t sharedExp = (uint64_t)(exp * 0.50);

			// Adiciona a XP na barra de progresso do Akamaru
			this->addAkamaruExperience(sharedExp);

			// O Player (Kiba) agora processa apenas a metade dele
			exp = sharedExp;
		}
	}
	// -------------------------------------------------------
	...
	experience += exp;
	uint32_t pointsToGive = 0; // Acumulador para a mensagem de avanço
	...
	while (experience >= nextLevelExp)
	{
		...
		// --- CÁLCULO DE PONTOS COM BÔNUS (10, 20, 30...) ---
		uint32_t currentGain = 0;
		if (level % 10 == 0)
		{
			currentGain = 3; // Ganha 3 pontos em níveis redondos
		}
		else
		{
			currentGain = 1; // Ganha 1 ponto nos demais níveis
		}

		this->addAttributePoints(currentGain); // Adiciona ao banco de dados/memória
		pointsToGive += currentGain;		   // Soma para exibir no final
	...
	if (prevLevel != level)
	{
		...
		// Mensagem de avanço utilizando os pontos acumulados no loop
		char advMsg[128];
		sprintf(advMsg, "Você avançou do nível %d para o nível %d e recebeu %d pontos de atributos.", prevLevel, level, pointsToGive);
		sendTextMessage(MSG_EVENT_ADVANCE, advMsg);

		if (client)
		{
			client->sendAttributePoints();
		}
...
void Player::onBlockHit(BlockType_t blockType)
{
	// reset de contador.
	bloodHitCount = 30;
	if (shieldBlockCount < 16)
	{
		shieldBlockCount = 16;
	}
	addSkillAdvance(SKILL_SHIELD, 10);
							 ...
							 bool checkDefense /* = false*/, bool checkArmor /* = false*/)
{
	...
	if (damage > 0)
	{
		...
		for (int32_t slot = SLOT_FIRST; slot < SLOT_LAST; ++slot)
		{
			if (!(item = getInventoryItem((slots_t)slot)) || (g_moveEvents->hasEquipEvent(item) && !isItemAbilityEnabled((slots_t)slot)))
				continue;
...
bool Player::onDeath()
{
	...
	// Se estiver em zona PVP, configuramos para não dropar loot e não perder skill
	if (getZone() == ZONE_PVP)
	{
	...
	else if (skull < SKULL_RED && g_game.getWorldType() != WORLD_TYPE_PVP_ENFORCED)
	{
		...
		{
			if (!(item = getInventoryItem((slots_t)i)) || (g_moveEvents->hasEquipEvent(item) && !isItemAbilityEnabled((slots_t)i)))
				continue;
	...
	// Lógica de itens que previnem perda (Amulet of Loss, etc)
	if (preventLoss)
	{
		setLossSkill(false);
		if (preventLoss->getCharges() > 1)
			g_game.transformItem(preventLoss, preventLoss->getID(), std::max((int32_t)0, ((int32_t)preventLoss->getCharges() - 1)));
		else
			g_game.internalRemoveItem(NULL, preventLoss);
	}

	if (preventDrop && preventDrop != preventLoss)
	{
		if (preventDrop->getCharges() > 1)
	...
	// Bloco de punição por morte
	if (skillLoss)
	{
		// --- BLOQUEIO DE PERDA DE NÍVEL (EXPERIÊNCIA) ---
		// Comentamos as linhas abaixo para que o level fique intacto
		/*
		uint64_t lossExperience = getLostExperience();
		removeExperience(lossExperience, false);
		*/

		// Mantemos o percent em 0 para que os cálculos abaixo não alterem nada
		double percent = 0.0;

		// --- BLOQUEIO DE PERDA DE MAGIC LEVEL ---
		// Como o percent é 0, o lostMana será 0 e nada mudará
		uint32_t sumMana = 0;
		for (uint32_t i = 1; i <= magLevel; ++i)
			sumMana += vocation->getReqMana(i);

		sumMana += manaSpent;
		uint64_t lostMana = (uint64_t)std::ceil(sumMana * ((double)(percent * lossPercent[LOSS_MANA]) / 100.));

		if (lostMana > (uint64_t)manaSpent)
			manaSpent = 0;
		else
			manaSpent -= (uint32_t)lostMana;
		...
		// --- BLOQUEIO DE PERDA DE SKILLS ---
		for (int16_t i = 0; i < 7; ++i)
		{
			continue; // Pula qualquer tentativa de reduzir skill
		}
		...
		// Cria a mochila de morte (container) se configurado, mas não tira level
...
void Player::onAttackedCreature(Creature *target)
{
	...
	if (Combat::isInPvpZone(this, targetPlayer) || isPartner(targetPlayer) || (g_config.getBool(ConfigManager::ALLOW_FIGHTBACK) && targetPlayer->hasAttacked(this)))
		return;
...
bool Player::onKilledCreature(Creature *target, uint32_t &flags)
{
	...
	if (!targetPlayer->hasAttacked(this) && target->getSkull() == SKULL_NONE && targetPlayer != this && ((g_config.getBool(ConfigManager::USE_FRAG_HANDLER) && addUnjustifiedKill(targetPlayer)) || hasBitSet((uint32_t)KILLFLAG_LASTHIT, flags)))
...
uint32_t Player::getAttackSpeed()
{
	...
	int32_t minimumAttackSpeed = 250;
	...
	int32_t attackSpeed = vocation->getAttackSpeed() - (getSkill(SKILL_CLUB, SKILL_LEVEL) * 5) - weaponAttackSpeed;
