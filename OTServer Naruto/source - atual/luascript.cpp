...
int32_t LuaScriptInterface::luaDoSendPlayerExtendedOpcode(lua_State* L)
{
    ...
    // A correção está aqui: acessamos o client/protocolo do player
    if(player->client)
    {
...
int32_t LuaScriptInterface::luaGetPlayerAkamaruAttack(lua_State* L) {
    ScriptEnviroment* env = getEnv();
    Player* player = env->getPlayerByUID(popNumber(L));
    if(player) lua_pushnumber(L, player->akamaruAttack); else lua_pushnil(L);
    return 1;
}
int32_t LuaScriptInterface::luaGetPlayerAkamaruAgility(lua_State* L) {
    ScriptEnviroment* env = getEnv();
    Player* player = env->getPlayerByUID(popNumber(L));
    if(player) lua_pushnumber(L, player->akamaruAgility); else lua_pushnil(L);
    return 1;
}
int32_t LuaScriptInterface::luaGetPlayerAkamaruDodge(lua_State* L) {
    ScriptEnviroment* env = getEnv();
    Player* player = env->getPlayerByUID(popNumber(L));
    if(player) lua_pushnumber(L, player->akamaruDodge); else lua_pushnil(L);
    return 1;
}
int32_t LuaScriptInterface::luaGetPlayerAkamaruHealthPts(lua_State* L) {
    ScriptEnviroment* env = getEnv();
    Player* player = env->getPlayerByUID(popNumber(L));
    if(player) lua_pushnumber(L, player->akamaruHealthPts); else lua_pushnil(L);
    return 1;
}
int32_t LuaScriptInterface::luaGetPlayerAkamaruSpeedPts(lua_State* L) {
    ScriptEnviroment* env = getEnv();
    Player* player = env->getPlayerByUID(popNumber(L));
    if(player) lua_pushnumber(L, player->akamaruSpeedPts); else lua_pushnil(L);
    return 1;
}
