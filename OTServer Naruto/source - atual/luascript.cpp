...
void LuaScriptInterface::error(const char *function, const std::string &desc)
{
	...
	{
		std::cout << std::endl
				  << "[Error - " << interface->getName() << "] " << std::endl;
		if (callback)
		...
		std::cout << interface->getScript(script) << std::endl
				  << "Description: ";
	}
	else
		std::cout << std::endl
...
void LuaScriptInterface::registerFunctions()
{
	...
	// createJuujinBunshin(cid, target_uid)
	lua_register(m_luaState, "createJuujinBunshin", LuaScriptInterface::luaCreateJuujinBunshin);

	// getPlayerAkamaruLevel(cid)
	lua_register(m_luaState, "getPlayerAkamaruLevel", LuaScriptInterface::luaGetPlayerAkamaruLevel);
	// getPlayerAkamaruExp()
	lua_register(m_luaState, "getPlayerAkamaruExp", LuaScriptInterface::luaGetPlayerAkamaruExp);
	// getPlayerAkamaruPoints()
	lua_register(m_luaState, "getPlayerAkamaruPoints", LuaScriptInterface::luaGetPlayerAkamaruPoints);
	// getPlayerAkamaruAttack()
	lua_register(m_luaState, "getPlayerAkamaruAttack", LuaScriptInterface::luaGetPlayerAkamaruAttack);
	// getPlayerAkamaruAgility()
	lua_register(m_luaState, "getPlayerAkamaruAgility", LuaScriptInterface::luaGetPlayerAkamaruAgility);
	// getPlayerAkamaruDodge()
	lua_register(m_luaState, "getPlayerAkamaruDodge", LuaScriptInterface::luaGetPlayerAkamaruDodge);
	// getPlayerAkamaruHealthPts()
	lua_register(m_luaState, "getPlayerAkamaruHealthPts", LuaScriptInterface::luaGetPlayerAkamaruHealthPts);
	// getPlayerAkamaruSpeedPts()
	lua_register(m_luaState, "getPlayerAkamaruSpeedPts", LuaScriptInterface::luaGetPlayerAkamaruSpeedPts);
	lua_register(m_luaState, "setPlayerAkamaruAttack", LuaScriptInterface::luaSetPlayerAkamaruAttack);
	lua_register(m_luaState, "setPlayerAkamaruAgility", LuaScriptInterface::luaSetPlayerAkamaruAgility);
	lua_register(m_luaState, "setPlayerAkamaruDodge", LuaScriptInterface::luaSetPlayerAkamaruDodge);
	lua_register(m_luaState, "setPlayerAkamaruHealthPts", LuaScriptInterface::luaSetPlayerAkamaruHealthPts);
	lua_register(m_luaState, "setPlayerAkamaruPoints", LuaScriptInterface::luaSetPlayerAkamaruPoints);
	lua_register(m_luaState, "getPlayerAkamaruNextLevelExp", LuaScriptInterface::luaGetPlayerAkamaruNextLevelExp);
	lua_register(m_luaState, "getPlayerAkamaruMaxHealth", LuaScriptInterface::luaGetPlayerAkamaruMaxHealth);
	lua_register(m_luaState, "getPlayerAkamaruMaxSpeed", LuaScriptInterface::luaGetPlayerAkamaruMaxSpeed);
	lua_register(m_luaState, "sendAkamaruData", LuaScriptInterface::luaSendAkamaruData);
	lua_register(m_luaState, "getAkamaruState", LuaScriptInterface::luaGetAkamaruState);
	lua_register(m_luaState, "reviveAkamaru", LuaScriptInterface::luaGetReviveAkamaru);
	lua_register(m_luaState, "setAkamaruOutfit", LuaScriptInterface::luaSetAkamaruOutfit);
	lua_register(m_luaState, "getAkamaruOutfit", LuaScriptInterface::luaGetAkamaruOutfit);

	// envio de dados para o otclient
	lua_register(m_luaState, "doSendPlayerExtendedOpcode", LuaScriptInterface::luaDoSendPlayerExtendedOpcode);

	// createBunshin(monsterName, pos, masterCid)
	lua_register(m_luaState, "createBunshin", LuaScriptInterface::luaCreateBunshin);
	...
	// doPlayerRemoveMana(cid, amount)
	lua_register(m_luaState, "doPlayerRemoveMana", LuaScriptInterface::luaDoPlayerRemoveMana);
...
int32_t LuaScriptInterface::luaCreateJuujinBunshin(lua_State *L)
{
	uint32_t target_uid = popNumber(L);
	uint32_t cid = popNumber(L);

	ScriptEnviroment *env = getEnv();
	Creature *creature = env->getCreatureByUID(cid);
	Creature *target = env->getCreatureByUID(target_uid);

	if (creature && target)
	{
		Player *player = creature->getPlayer();
		Monster *summon = target->getMonster();

		if (player && summon)
		{
			if (summon->getName() == player->getName())
			{
				// DESTRANSFORMA
				summon->name = summon->getMonsterType()->name;
				summon->nameDescription = summon->getMonsterType()->nameDescription;
				g_game.internalCreatureChangeOutfit(summon, summon->getMonsterType()->outfit);
				summon->setBaseSpeed(summon->getMonsterType()->baseSpeed);
			}
			else
			{
				// TRANSFORMA
				summon->name = player->getName();
				summon->nameDescription = player->getNameDescription();
				// Na sua source a propriedade ï¿½ defaultOutfit (confirmado em erros anteriores)
				g_game.internalCreatureChangeOutfit(summon, player->defaultOutfit);
				summon->setBaseSpeed(player->getBaseSpeed());
			}

			g_game.changeSpeed(summon, 0);
			g_game.addMagicEffect(summon->getPosition(), 2);

			// ATUALIZAï¿½ï¿½O DE NOME E VISUAL (Protocolo Antigo)
			for (AutoList<Player>::iterator it = Player::autoList.begin(); it != Player::autoList.end(); ++it)
			{
				if (it->second->canSeeCreature(summon))
				{
					// O erro na imagem 9 confirmou que a funï¿½ï¿½o pede (const Creature*, Visible_t)
					// Em sources antigas, o valor 3 representa a visibilidade total (Full)
					it->second->sendCreatureChangeVisible(summon, (Visible_t)3);
				}
			}
...
int32_t LuaScriptInterface::luaDoSendPlayerExtendedOpcode(lua_State *L)
{
	...
	// A correï¿½ï¿½o estï¿½ aqui: acessamos o client/protocolo do player
	if (player->client)
	{
		player->client->sendExtendedOpcode(opcode, buffer);
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaScriptInterface::luaGetPlayerAkamaruLevel(lua_State *L)
{
	// getPlayerAkamaruLevel(cid)
	ScriptEnviroment *env = getEnv();
	Player *player = env->getPlayerByUID(popNumber(L));
	if (player)
		lua_pushnumber(L, player->akamaruLevel);
	else
		lua_pushnil(L);
	return 1;
}
int32_t LuaScriptInterface::luaGetPlayerAkamaruExp(lua_State *L)
{
	ScriptEnviroment *env = getEnv();
	Player *player = env->getPlayerByUID(popNumber(L));
	if (player)
		lua_pushnumber(L, player->akamaruExp);
	else
		lua_pushnil(L);
	return 1;
}
int32_t LuaScriptInterface::luaGetPlayerAkamaruPoints(lua_State *L)
{
	ScriptEnviroment *env = getEnv();
	Player *player = env->getPlayerByUID(popNumber(L));
	if (player)
		lua_pushnumber(L, player->akamaruPoints);
	else
		lua_pushnil(L);
	return 1;
}
int32_t LuaScriptInterface::luaGetPlayerAkamaruAttack(lua_State *L)
{
	ScriptEnviroment *env = getEnv();
	Player *player = env->getPlayerByUID(popNumber(L));
	if (player)
		lua_pushnumber(L, player->akamaruAttack);
	else
		lua_pushnil(L);
	return 1;
}
int32_t LuaScriptInterface::luaGetPlayerAkamaruAgility(lua_State *L)
{
	ScriptEnviroment *env = getEnv();
	Player *player = env->getPlayerByUID(popNumber(L));
	if (player)
		lua_pushnumber(L, player->akamaruAgility);
	else
		lua_pushnil(L);
	return 1;
}
int32_t LuaScriptInterface::luaGetPlayerAkamaruDodge(lua_State *L)
{
	ScriptEnviroment *env = getEnv();
	Player *player = env->getPlayerByUID(popNumber(L));
	if (player)
		lua_pushnumber(L, player->akamaruDodge);
	else
		lua_pushnil(L);
	return 1;
}
int32_t LuaScriptInterface::luaGetPlayerAkamaruHealthPts(lua_State *L)
{
	ScriptEnviroment *env = getEnv();
	Player *player = env->getPlayerByUID(popNumber(L));
	if (player)
		lua_pushnumber(L, player->akamaruHealthPts);
	else
		lua_pushnil(L);
	return 1;
}
int32_t LuaScriptInterface::luaGetPlayerAkamaruSpeedPts(lua_State *L)
{
	ScriptEnviroment *env = getEnv();
	Player *player = env->getPlayerByUID(popNumber(L));
	if (player)
		lua_pushnumber(L, player->akamaruSpeedPts);
	else
		lua_pushnil(L);
	return 1;
}
int32_t LuaScriptInterface::luaSetPlayerAkamaruAttack(lua_State *L)
{
	uint32_t value = popNumber(L);
	ScriptEnviroment *env = getEnv();
	Player *player = env->getPlayerByUID(popNumber(L));
...
int32_t LuaScriptInterface::luaSetPlayerAkamaruAgility(lua_State *L)
{
	uint32_t value = popNumber(L);
	ScriptEnviroment *env = getEnv();
	Player *player = env->getPlayerByUID(popNumber(L));
...
int32_t LuaScriptInterface::luaSetPlayerAkamaruDodge(lua_State *L)
{
	uint32_t value = popNumber(L);
	ScriptEnviroment *env = getEnv();
	Player *player = env->getPlayerByUID(popNumber(L));
...
int32_t LuaScriptInterface::luaSetPlayerAkamaruHealthPts(lua_State *L)
{
	// setPlayerAkamaruHealthPts(cid, newAmount)
	uint32_t newAmount = popNumber(L);
	ScriptEnviroment *env = getEnv();
	Player *player = env->getPlayerByUID(popNumber(L));
	if (player)
	{
		player->akamaruHealthPts = newAmount;
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaScriptInterface::luaSetPlayerAkamaruPoints(lua_State *L)
{
	uint32_t newAmount = popNumber(L);
	ScriptEnviroment *env = getEnv();
	Player *player = env->getPlayerByUID(popNumber(L));
	if (player)
	{
		player->akamaruPoints = newAmount;
		lua_pushboolean(L, true);
	}
	else
	{
...
int32_t LuaScriptInterface::luaGetPlayerAkamaruNextLevelExp(lua_State *L)
{
	uint32_t playerId = popNumber(L);
	ScriptEnviroment *env = getEnv();
	if (Player *player = env->getPlayerByUID(playerId))
	{
		lua_pushnumber(L, (double)player->getAkamaruRequiredExpToNextLevel());
	}
	else
	{
		lua_pushnil(L);
...
int32_t LuaScriptInterface::luaGetPlayerAkamaruMaxHealth(lua_State *L)
{
	uint32_t playerId = popNumber(L);
	ScriptEnviroment *env = getEnv();
	...
	if (Player *player = env->getPlayerByUID(playerId))
	{
		lua_pushnumber(L, player->getAkamaruMaxHealth());
	}
	else
	{
		lua_pushnil(L);
...
int32_t LuaScriptInterface::luaGetPlayerAkamaruMaxSpeed(lua_State *L)
{
	uint32_t playerId = popNumber(L);
	ScriptEnviroment *env = getEnv();
	...
	if (Player *player = env->getPlayerByUID(playerId))
	{
		lua_pushnumber(L, player->getAkamaruMaxSpeed());
	}
	else
	{
		lua_pushnil(L);
...
// envia dados do akamaru para atulizar o client
int32_t LuaScriptInterface::luaSendAkamaruData(lua_State *L)
{
	// sendAkamaruData(cid)
	uint32_t cid = popNumber(L);
...
int32_t LuaScriptInterface::luaGetAkamaruState(lua_State *L)
{
	// getAkamaruState(cid)
	uint32_t cid = popNumber(L);
	ScriptEnviroment *env = getEnv();
	...
	// Primeiro retorno: isDead (boolean)
	lua_pushboolean(L, player->akamaruIsDead);

	// Cálculo do tempo restante
	uint32_t waitTime = player->akamaruLevel * 10; // 10 segundos por nível
	uint32_t currentTime = (uint32_t)time(NULL);
	int32_t timeLeft = (int32_t)((player->akamaruDeathTimestamp + waitTime) - currentTime);

	// Segundo retorno: timeLeft (número)
	lua_pushnumber(L, (timeLeft > 0 ? timeLeft : 0));
	lua_pushnumber(L, player->akamaruCurrentHp);
	return 3; // Retorna três valores para o Lua
}

int32_t LuaScriptInterface::luaGetReviveAkamaru(lua_State *L)
{
	// reviveAkamaru(cid)
	uint32_t cid = popNumber(L);
	ScriptEnviroment *env = getEnv();
	...
	// Reseta o estado na Source
	player->updateAkamaruStatus(100, false); // Define como vivo com HP base

	lua_pushboolean(L, true);
	return 1;
}

// setAkamaruOutfit(cid, looktype)
int32_t LuaScriptInterface::luaSetAkamaruOutfit(lua_State* L)
{
    uint32_t looktype = popNumber(L);
    uint32_t cid = popNumber(L);

    // Adicionamos a declaração do env igual na função de reviver:
    ScriptEnviroment *env = getEnv();

    Player* player = env->getPlayerByUID(cid);
    if(player)
    {
        player->akamaruOutfit = looktype;
        lua_pushboolean(L, true);
    }
    else
    {
...
// getAkamaruOutfit(cid)
int32_t LuaScriptInterface::luaGetAkamaruOutfit(lua_State* L)
{
    uint32_t cid = popNumber(L);
    ...
    if(player)
    {
        lua_pushnumber(L, player->akamaruOutfit);
    }
    else
    {
        lua_pushboolean(L, false);
...
int32_t LuaScriptInterface::luaCreateBunshin(lua_State *L)
{
	uint32_t masterCid = popNumber(L);
	PositionEx pos;
	popPosition(L, pos);
	std::string monsterName = popString(L);

	ScriptEnviroment *env = getEnv();
	Creature *master = env->getCreatureByUID(masterCid);
	if (!master)
	{
		lua_pushboolean(L, false);
		return 1;
	...
	Monster *monster = Monster::createMonster(monsterName.c_str());
	if (!monster)
	{
		lua_pushboolean(L, false);
		return 1;
	...
	monster->setName(master->getName());
	monster->setNameDescription("a " + master->getName());
	monster->defaultOutfit = master->getCurrentOutfit();
	monster->currentOutfit = master->getCurrentOutfit();
	...
	int32_t targetSpeed = (int32_t)master->getSpeed();

	monster->setBaseSpeed(0);

	int32_t speedDiff = targetSpeed - (int32_t)monster->getSpeed();

	g_game.changeSpeed(monster, speedDiff);
...
int32_t LuaScriptInterface::luaDoPlayerRemoveMana(lua_State *L)
{
	int32_t amount = std::abs(popNumber(L)); // caotura o valor positivo.
	uint32_t cid = popNumber(L);
	...
	uint32_t manaBefore = player->getMana();

	// metodo oficial de remoï¿½ï¿½o de mana do player.
	player->changeMana(-amount);

	uint32_t manaAfter = player->getMana();
	int32_t actuallyRemoved = manaBefore - manaAfter;

	if (actuallyRemoved > 0)
	{
		lua_pushnumber(L, actuallyRemoved); // retorna o consumo
	}
	else
	{
		lua_pushboolean(L, false);
