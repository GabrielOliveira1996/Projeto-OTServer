...
void loadProtections() {
    ...
    monsterProtections["Mizuki"] = 11112;
    monsterProtections["Ebisu"] = 11117;
    monsterProtections["Zabuza"] = 11120;
    monsterProtections["Second Zabuza"] = 11121;
    monsterProtections["Haku"] = 11122;
    monsterProtections["Orochimaru"] = 11000;
    monsterProtections["Zaku"] = 11127;
    monsterProtections["Kin"] = 11128;
    monsterProtections["Dosu"] = 11129;
    monsterProtections["Rain Shinobi"] = 11131;
    monsterProtections["Kiba"] = 11133;
    monsterProtections["Neji"] = 11136;
    monsterProtections["Kankuro"] = 11138;
    monsterProtections["Temari"] = 11139;
    monsterProtections["Gaara"] = 11140;
    monsterProtections["Shukaku One"] = 11141;
    monsterProtections["Shukaku Two"] = 11142;
    monsterProtections["Kabuto"] = 11146;
    monsterProtections["Sasuke"] = 11148;
    monsterProtections["Jiroubou"] = 11151;
    monsterProtections["Jiroubou Cursed"] = 11152;
    monsterProtections["Kidomaru"] = 11153;
    monsterProtections["Kidomaru Cursed"] = 11154;
    monsterProtections["Sakon And Ukon"] = 11155;
    monsterProtections["Sakon And Ukon Cursed"] = 11156;
    monsterProtections["Tayuya"] = 11157;
    monsterProtections["Tayuya Cursed"] = 11158;
    monsterProtections["Kimimaru"] = 11159;
    monsterProtections["Kimimaru Cursed"] = 11160;
...
bool canPlayerAttackMonster(const Player* player, const Creature* target) {
    ...
    if (it != monsterProtections.end()) {
        std::string value;
        uint32_t requiredStorage = it->second;
        // Se getStorage retornar false, o player definitivamente não tem a storage
        if (!const_cast<Player*>(player)->getStorage(requiredStorage, value)) {
            return false;
        }
        // Verifica se o valor é válido (maior que 0, por exemplo, se você usa 1 para 'completo')
        // Se sua lógica usa 1 para permitir, mude para: atoi(value.c_str()) <= 0
        if (atoi(value.c_str()) <= -1) { 
            return false;
        }
    }
    return true; // Retorna true se o monstro não está na lista OU se o player passou nos testes
}
...
ReturnValue Combat::canDoCombat(const Creature* caster, const Tile* tile, bool isAggressive)
{
    ...
    {
        // Pega a criatura que está no SQM (Tile)
        if(const Creature* target = const_cast<Tile*>(tile)->getTopCreature())
        {
            // Extraímos o ponteiro do monstro uma única vez para usar em tudo
            const Monster* targetMonster = target->getMonster();
    
            if(caster->getMonster() && targetMonster)
            {
                // Verifica se o alvo é um summon
                if(targetMonster->isSummon())
                {
                    const Creature* master = targetMonster->getMaster();
                    // Se o alvo tem um mestre e esse mestre é um PLAYER, PERMITE o dano
                    if(master && master->getPlayer())
                    {
                        // Não faz nada, deixa passar para o dano ocorrer
                    }
                    else
                    {
                        // Se o mestre não existe ou é outro monstro, bloqueia
                        return RET_YOUMAYNOTATTACKTHISCREATURE;
                    }
                }
                else
                {
                    // Se o alvo NÃO é summon (é monstro selvagem), bloqueia o dano vindo de monstro
                    return RET_YOUMAYNOTATTACKTHISCREATURE;
                }
            }
    
            // --- Proteção para o caster não acertar o próprio summon com área ---
...
ReturnValue Combat::canDoCombat(const Creature* attacker, const Creature* target)
{
	...
	{
		// Impede o início do ataque e a seleção do quadrado vermelho
		return RET_YOUMAYNOTATTACKTHISCREATURE;
	}
