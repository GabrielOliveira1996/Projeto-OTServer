...
bool IOLoginData::loadPlayer(Player *player, const std::string &name, bool preLoad /*= false*/)
{
	Database *db = Database::getInstance();
	DBQuery query;
	query << "SELECT `id`, `account_id`, `group_id`, `world_id`, `sex`, `vocation`, `experience`, `level`, `attribute_points`, `maglevel`, ";
	...
	query << "`loss_items`, `marriage`, `promotion`, `description` ";
	...
	player->attributePoints = (uint32_t)result->getDataInt("attribute_points");
	...
	if (player->isKiba())
	{
		std::stringstream akamaruQuery;
		akamaruQuery << "SELECT `level`, `exp`, `points`, `attack`, `agility`, `dodge`, `health_pts`, `speed_pts`, `current_hp`, `is_dead`, `death_timestamp`, `current_outfit` "
					 << "FROM `player_akamarus` WHERE `player_id` = " << player->getGUID();

		DBResult *akamaruResult = db->storeQuery(akamaruQuery.str());
		if (akamaruResult)
		{
			player->akamaruLevel = akamaruResult->getDataInt("level");
			player->akamaruExp = akamaruResult->getDataLong("exp");
			player->akamaruPoints = akamaruResult->getDataInt("points");
			player->akamaruAttack = akamaruResult->getDataInt("attack");
			player->akamaruAgility = akamaruResult->getDataInt("agility");
			player->akamaruDodge = akamaruResult->getDataInt("dodge");
			player->akamaruHealthPts = akamaruResult->getDataInt("health_pts");
			player->akamaruSpeedPts = akamaruResult->getDataInt("speed_pts");
			player->akamaruCurrentHp = akamaruResult->getDataInt("current_hp");
			player->akamaruIsDead = (akamaruResult->getDataInt("is_dead") != 0);
			player->akamaruDeathTimestamp = (uint32_t)akamaruResult->getDataLong("death_timestamp");
			player->akamaruOutfit = akamaruResult->getDataInt("current_outfit");
			akamaruResult->free();
		}
	}
	...
	if ((result = db->storeQuery(query.str())))
	{
		...
		do
		{
			...
			uint64_t nextSkillCount = player->vocation->getReqSkillTries(
					 ...
					 skillCount = result->getDataLong("count");
...
bool IOLoginData::savePlayer(Player *player, bool preSave /* = true*/, bool shallow /* = false*/)
{
	...
	query << "`attribute_points` = " << player->getAttributePoints() << ", ";
	...
	if (player->isKiba())
	{
		std::stringstream akamaruSave;
		akamaruSave << "INSERT INTO `player_akamarus` (`player_id`, `level`, `exp`, `points`, `attack`, `agility`, `dodge`, `health_pts`, `speed_pts`, `current_hp`, `is_dead`, `death_timestamp`, `current_outfit`) VALUES ("
					<< player->getGUID() << ", "
					<< player->akamaruLevel << ", "
					<< player->akamaruExp << ", "
					<< player->akamaruPoints << ", "
					<< player->akamaruAttack << ", "
					<< player->akamaruAgility << ", "
					<< player->akamaruDodge << ", "
					<< player->akamaruHealthPts << ", "
					<< player->akamaruSpeedPts << ", "
					<< player->akamaruCurrentHp << ", "
					<< (player->akamaruIsDead ? 1 : 0) << ", "
					<< player->akamaruDeathTimestamp << ", "
					<< player->akamaruOutfit << ") "
					<< "ON DUPLICATE KEY UPDATE "
					<< "`level` = VALUES(`level`), `exp` = VALUES(`exp`), `points` = VALUES(`points`), "
					<< "`attack` = VALUES(`attack`), `agility` = VALUES(`agility`), `dodge` = VALUES(`dodge`), "
					<< "`health_pts` = VALUES(`health_pts`), `speed_pts` = VALUES(`speed_pts`), "
					<< "`current_hp` = VALUES(`current_hp`), "
					<< "`is_dead` = VALUES(`is_dead`), "
					<< "`death_timestamp` = VALUES(`death_timestamp`), "
					<< "`current_outfit` = VALUES(`current_outfit`);";

		db->executeQuery(akamaruSave.str());
	}
