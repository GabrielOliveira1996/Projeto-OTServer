	...
	Creature()
{
    ...
    if(mType)
    {
        ...
        this->Creature::nameDescription = mType->nameDescription; // puxa a descrição
    }
	spawn = NULL;
	raid = NULL;
...
bool Monster::isTarget(Creature* creature)
{
    ...
    {
        // Usamos a função global para checar se este jogador tem permissão
        // para interagir com este monstro (this)
        if(!canPlayerAttackMonster(targetPlayer, this))
        {
            // Se o player não tem a storage necessária para este monstro,
            // o monstro ignora o player (não ataca e não persegue)
            return false;
        }
	}
    return true;
...
void Monster::doAttacking(uint32_t interval)
{
	...
	// --- INÍCIO: APLICAÇÃO DA AGILIDADE NO TOPO ---
	uint32_t agilityBonus = 0;
	if (getName() == "Akamaru" && isSummon() && master) {
		Player* playerMaster = master->getPlayer();
		if (playerMaster) {
			// Cada ponto reduz 10ms do intervalo (Limite de 1700ms)
			agilityBonus = std::min((uint32_t)1700, (uint32_t)(playerMaster->akamaruAgility * 10));
			
			// Garantimos que o intervalo resultante não seja menor que 300ms
			if (interval > 300 && (interval - agilityBonus) < 300) {
				agilityBonus = interval - 300;
			}
		}
	}
	
	// Adicionamos o intervalo + o bônus de agilidade para "acelerar" o relógio
	...
	for(SpellList::iterator it = mType->spellAttackList.begin(); it != mType->spellAttackList.end(); ++it)
	{
		...
		if(canUseSpell(myPos, targetPos, *it, interval, inRange))
		{
			if(it->chance >= (uint32_t)random_range(1, 100))
			{
				...
				// Bônus de Dano do Akamaru
				if (getName() == "Akamaru" && isSummon() && master) {
					Player* playerMaster = master->getPlayer();
					if (playerMaster) {
						float attackPts = (float)playerMaster->akamaruAttack; 
						minCombatValue -= (int32_t)(attackPts * 0.5f);
						maxCombatValue -= (int32_t)(attackPts * 1.5f);
					}
				}
...
{ 
    ...
    if(target)
    {
        ...
        if(targetPlayer)
        {
            // Se o monstro for um dos "Mestres" e o player não tiver a storage
            if(!canPlayerAttackMonster(targetPlayer, this))
            {
    ...
    {
        // Se não houver alvo e o monstro for um dos protegidos, 
        // podemos impedir o movimento aleatório (opcional, igual você fez no Mizuki)
        // Usamos uma verificação simples: o monstro está na lista?
