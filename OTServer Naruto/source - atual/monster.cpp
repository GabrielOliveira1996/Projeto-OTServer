...
Monster::Monster(MonsterType *_mType) : Creature()
{
	...
	if (mType)
	{
		this->Creature::name = mType->name;						  // puxa o nome do XML
		this->Creature::nameDescription = mType->nameDescription; // puxa a descriï¿½ï¿½o
	}
	spawn = NULL;
	raid = NULL;
...
{
	return (isSummon() && getMaster()->getPlayer() && creature != getMaster()) || ((creature->getPlayer() && !creature->getPlayer()->hasFlag(PlayerFlag_IgnoredByMonsters)) ||
...
{
	if (!creature || creature->isRemoved() || !creature->isAttackable() ||
		creature->getZone() == ZONE_PROTECTION || !canSeeCreature(creature) ||
		creature->getPosition().z != getPosition().z)
	{
		return false;
	}
	// VerificaÃ§Ã£o de Storage para Players
	if (Player *targetPlayer = creature->getPlayer())
	{
		// Usamos a funï¿½ï¿½o global para checar se este jogador tem permissï¿½o
		// para interagir com este monstro (this)
		if (!canPlayerAttackMonster(targetPlayer, this))
		{
			// Se o player nï¿½o tem a storage necessï¿½ria para este monstro,
			// o monstro ignora o player (nï¿½o ataca e nï¿½o persegue)
			return false;
		}
	}
	return true;
...
void Monster::doAttacking(uint32_t interval)
{
	...
	uint32_t agilityBonus = 0;

	// Verifica se é um summon e se possui a storage que o identifica como Akamaru
    ...
    if (isSummon() && master && getStorage(STORAGE_AKAMARU_IDENTIFIER, akamaruValue) && atoi(akamaruValue.c_str()) == 1)
	{
		...
		if (playerMaster)
		{
			// O bônus de agilidade acelera os attackTicks
			agilityBonus = playerMaster->akamaruAgility * 15;
		}
	}

	attackTicks += (interval + agilityBonus);
	...
	for (SpellList::iterator it = mType->spellAttackList.begin(); it != mType->spellAttackList.end(); ++it)
	{
		...
		bool inRange = false;
		if (canUseSpell(myPos, targetPos, *it, attackTicks, inRange))
		{
			if (it->chance >= (uint32_t)random_range(1, 100))
			{
				...
				// --- Lógica Dinâmica de Dano do Akamaru ---
				std::string akamaruValue = "";
                if (isSummon() && master && getStorage(STORAGE_AKAMARU_IDENTIFIER, akamaruValue) && atoi(akamaruValue.c_str()) == 1)
				{
					Player *playerMaster = master->getPlayer();
					if (playerMaster)
					{
						float attackPts = (float)playerMaster->akamaruAttack;
						// Aplica o bônus de dano baseado nos pontos do mestre
						minCombatValue -= (int32_t)(attackPts * 1.0f);
						maxCombatValue -= (int32_t)(attackPts * 2.0f);
					}
				}
...
bool Monster::getNextStep(Direction &dir, uint32_t &flags)
{
	...
	if (target)
	{
		...
		if (targetPlayer)
		{
			// Se o monstro for um dos "Mestres" e o player nï¿½o tiver a storage
			if (!canPlayerAttackMonster(targetPlayer, this))
			{
	...
	{
		// Se nï¿½o houver alvo e o monstro for um dos protegidos,
		// podemos impedir o movimento aleatï¿½rio (opcional, igual vocï¿½ fez no Mizuki)
		// Usamos uma verificaï¿½ï¿½o simples: o monstro estï¿½ na lista?
...
{
	std::string akamaruValue = "";
    if (damage > 0 && isSummon() && master && getStorage(STORAGE_AKAMARU_IDENTIFIER, akamaruValue) && atoi(akamaruValue.c_str()) == 1)
    {
        Player *playerMaster = master->getPlayer();
        if (playerMaster)
        {
            uint32_t dodgeChance = std::min((uint32_t)35, (uint32_t)(playerMaster->akamaruDodge / 10));
...
bool Monster::convinceCreature(Creature *creature)
{
	 ...
	 * Substituída a verificação de nome (getName() == "Akamaru") pela Storage Identifier.
	 * Isso permite que o Akamaru mantenha seus bônus mesmo transformado (Juujin Bunshin).
	 */
	std::string akamaruStr = "";
    if (getStorage(STORAGE_AKAMARU_IDENTIFIER, akamaruStr) && atoi(akamaruStr.c_str()) == 1)
    {
		this->isAkamaru = true;
		Player *playerMaster = creature->getPlayer();

		if (playerMaster)
		{
			// Verifica se o novo mestre pertence à vocação Inuzuka (IDs 33 a 37)
			uint32_t vocId = playerMaster->getVocationId();
			if (vocId >= 33 && vocId <= 37)
			{
				// Aplica o HP máximo baseado nos pontos de status do mestre
				uint32_t maxHp = playerMaster->getAkamaruMaxHealth();
				this->healthMax = maxHp;
				this->health = maxHp;
				g_game.addCreatureHealth(this);

				// Aplica a velocidade de movimento baseada nos pontos do mestre
				uint32_t newSpeed = playerMaster->getAkamaruMaxSpeed();

				// Reseta variações para evitar que a velocidade acumule indevidamente
				this->varSpeed = 0;
				this->baseSpeed = newSpeed;

				// Calcula a diferença e força a atualização da velocidade no cliente
				int32_t speedDelta = newSpeed - getSpeed();
	...
	/**
	 * [INJEÇÃO DE COMBATE IMEDIATO]:
	 * Faz com que o summon ataque imediatamente o alvo do mestre,
	 * tirando-o do estado de 'idle' (paralisia inicial).
	 */
	if (Creature *masterTarget = creature->getAttackedCreature())
	{
		if (masterTarget != this && masterTarget != creature)
		{
			this->setAttackedCreature(masterTarget);
			this->setFollowCreature(masterTarget);
			this->isIdle = false;
		}
	}

	// Notifica os espectadores sobre a mudança de mestre
	SpectatorVec list;
