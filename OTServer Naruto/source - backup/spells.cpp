...
// necessário para cooldown para navbar
#include <sstream>
...
bool Spell::configureSpell(xmlNodePtr p)
{
	...
	// Lê o ID do ícone (itemid) (ex: icon="3147")
	if (readXMLInteger(p, "icon", intValue))
		iconId = (uint16_t)intValue;

	// Lê a descrição da spell (ex: description="Invoca clones...")
...
void Spell::postCastSpell(Player *player, bool finishedCast /*= true*/, bool payCost /*= true*/) const
{
	if (finishedCast)
	{
		...
		// Opcode 101 para Cooldowns (padrão Mehah)
		uint8_t opcode = 101;
		std::stringstream buffer;

		// Enviamos: Nome da Magia | Tempo de Exaustão (ms)
		// Se sua source tiver cooldown individual, use a variável dele aqui.
		// Se usar apenas o exhaustion padrão:
...
int32_t Spell::getManaCost(const Player *player) const
{
	...
	int32_t limit = baseMana / 2;				  // o custo n�o pode cair mais que 50%
	int32_t discounted = baseMana - fishingSkill; // aqui � subtra��o porque baseMana � positivo
