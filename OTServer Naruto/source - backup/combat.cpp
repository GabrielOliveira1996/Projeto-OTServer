...
std::map<std::string, uint32_t> monsterProtections;
...
void loadProtections()
{
	...
	// Saga de Introdução e Mizuki
	monsterProtections["Mizuki"] = 4; // SAGA_STAGE_IRUKA_RECOGNITION

	// Saga do Ebisu / Konohamaru
	monsterProtections["Ebisu"] = 7; // SAGA_STAGE_EBISU_CHALLENGE

	// Saga do País das Ondas
	monsterProtections["Gozu"] = 10;						 // SAGA_STAGE_WAVES_ESCORT
	monsterProtections["Meizu"] = 10;						 // SAGA_STAGE_WAVES_ESCORT
	monsterProtections["Zabuza Momochi [Hidden Mist]"] = 10; // SAGA_STAGE_BRIDGE_AMBUSH

	monsterProtections["Haku"] = 15;							// SAGA_STAGE_BRIDGE_AMBUSH
	monsterProtections["Zabuza Momochi [The Final Mist]"] = 15; // SAGA_STAGE_HIDDEN_GAS_DEMON

	// Exame Chunin - Floresta da Morte
	monsterProtections["Orochimaru"] = 19; // SAGA_STAGE_SERPENT_ENCOUNTER
	monsterProtections["Zaku"] = 21;	   // SAGA_STAGE_DEFENSE_ZAKU
	monsterProtections["Kin"] = 22;		   // SAGA_STAGE_DEFENSE_KIN
	monsterProtections["Dosu"] = 23;	   // SAGA_STAGE_DEFENSE_DOSU
	monsterProtections["Suna Ally"] = 24;  // Ex: Kankuro/Temari na floresta
	/// monsterProtections["Rain Shinobi"] = 13;  // SAGA_STAGE_WAVES_MIST

	// Preliminares e Treinamento
	monsterProtections["Kiba"] = 27; // SAGA_STAGE_PRELIMINARY_KIBA
	monsterProtections["Neji"] = 30; // SAGA_STAGE_DESTINY_VS_EFFORT

	// Invasão de Konoha
	monsterProtections["Kankuro"] = 32;		// SAGA_STAGE_PURSUIT_KANKURO
	monsterProtections["Temari"] = 33;		// SAGA_STAGE_PURSUIT_TEMARI
	monsterProtections["Gaara"] = 34;		// SAGA_STAGE_JINCHUURIKI_AWAKENING
	monsterProtections["Shukaku One"] = 35; // SAGA_STAGE_SHUKAKU_FORM1
	monsterProtections["Shukaku Two"] = 36; // SAGA_STAGE_BEAST_CONFRONTATION

	// Busca por Tsunade
	monsterProtections["Kabuto"] = 39; // SAGA_STAGE_KABUTO_ESPIONAGE
	// Reutilizando Orochimaru se houver uma segunda luta:
	// monsterProtections["Orochimaru V2"] = 40;

	// Resgate do Sasuke
	monsterProtections["Sasuke"] = 42;				  // SAGA_STAGE_EXPLOSIVE_RIVALRY (Hospital ou Vale)
	monsterProtections["Jiroubou"] = 45;			  // SAGA_STAGE_SOUND_JIROUBOU
	monsterProtections["Jiroubou Cursed"] = 46;		  // SAGA_STAGE_CURSE_MARK_POWER
	monsterProtections["Kidomaru"] = 47;			  // SAGA_STAGE_SOUND_KIDOMARU
	monsterProtections["Kidomaru Cursed"] = 48;		  // SAGA_STAGE_SHADOW_HUNTER
	monsterProtections["Sakon And Ukon"] = 49;		  // SAGA_STAGE_TWINS_SAKON_UKON
	monsterProtections["Sakon And Ukon Cursed"] = 50; // SAGA_STAGE_TWO_HEADED_NIGHTMARE
	monsterProtections["Tayuya"] = 51;				  // SAGA_STAGE_DEATH_MELODY
	monsterProtections["Tayuya Cursed"] = 52;		  // SAGA_STAGE_CURSED_SYMPHONY
	monsterProtections["Kimimaru"] = 53;			  // SAGA_STAGE_LEADER_KIMIMARO
	monsterProtections["Kimimaru Cursed"] = 54;		  // SAGA_STAGE_CLEMATIS_DANCE

	// Vale do Fim Final
	// monsterProtections["Final Sasuke"] = 55; // SAGA_STAGE_VALLEY_OF_THE_END

	protectionsLoaded = true;
}

bool canPlayerAttackMonster(const Player *player, const Creature *target)
{
	if (!player || !target)
		return true;
	...
	// Em vez de 'auto', usamos o tipo completo do iterator
	std::map<std::string, uint32_t>::iterator it = monsterProtections.find(target->getName());

	if (it != monsterProtections.end())
	{
		std::string storageValueStr;
		uint32_t sagaStorage = 11000;		 // Sua storage fixa
		uint32_t requiredStage = it->second; // Valor vindo da lista (it->second)

		// Busca o valor da storage
		if (!const_cast<Player *>(player)->getStorage(sagaStorage, storageValueStr))
		{
			return false;
		}

		// Voltamos a usar o atoi() para converter a string em numero, ja que seu compilador nao tem stoul
		if (atoi(storageValueStr.c_str()) != (int32_t)requiredStage)
		{
			return false;
		}
	}
...
ReturnValue Combat::canDoCombat(const Creature *caster, const Tile *tile, bool isAggressive)
{
	...
	if (caster && isAggressive)
	{
		// Pega a criatura que est� no SQM (Tile)
		if (const Creature *target = const_cast<Tile *>(tile)->getTopCreature())
		{
			// Extra�mos o ponteiro do monstro uma �nica vez para usar em tudo
			const Monster *targetMonster = target->getMonster();

			if (caster->getMonster() && targetMonster)
			{
				// Verifica se o alvo � um summon
				if (targetMonster->isSummon())
				{
					const Creature *master = targetMonster->getMaster();
					// Se o alvo tem um mestre e esse mestre � um PLAYER, PERMITE o dano
					if (master && master->getPlayer())
					{
						// N�o faz nada, deixa passar para o dano ocorrer
					}
					else
					{
						// Se o mestre n�o existe ou � outro monstro, bloqueia
						return RET_YOUMAYNOTATTACKTHISCREATURE;
					}
				}
				else
				{
					// Se o alvo N�O � summon (� monstro selvagem), bloqueia o dano vindo de monstro
					return RET_YOUMAYNOTATTACKTHISCREATURE;
				}
			}

			// --- Prote��o para o caster n�o acertar o pr�prio summon com �rea ---
			if (targetMonster && targetMonster->getMaster() == caster)
			{
				if (const_cast<Creature *>(caster)->getAttackedCreature() != target)
				{
					return RET_YOUMAYNOTATTACKTHISCREATURE;
				}
			}
...
ReturnValue Combat::canDoCombat(const Creature *attacker, const Creature *target)
{
	...
	if (!canPlayerAttackMonster(attacker->getPlayer(), target))
	{
		// Impede o in�cio do ataque e a sele��o do quadrado vermelho
		return RET_YOUMAYNOTATTACKTHISCREATURE;
	}
	...
	return checkZones && (target->getTile()->hasFlag(TILESTATE_NOPVPZONE) ||
						  (attacker->getTile()->hasFlag(TILESTATE_NOPVPZONE) && !target->getTile()->hasFlag(TILESTATE_NOPVPZONE) &&
						   !target->getTile()->hasFlag(TILESTATE_PROTECTIONZONE)))
			   ? RET_ACTIONNOTPERMITTEDINANOPVPZONE
			   : RET_NOERROR;
}
...
{
	if (caster && target && !canPlayerAttackMonster(caster->getPlayer(), target))
	{
...
void MagicField::onStepInField(Creature *creature, bool purposeful /* = true*/)
{
	...
	if (ownerId && !getTile()->hasFlag(TILESTATE_PVPZONE))
	{
		...
		{
			bool harmful = true;
			if ((g_game.getWorldType() == WORLD_TYPE_NO_PVP || getTile()->hasFlag(TILESTATE_NOPVPZONE)) && (owner->getPlayer() || owner->isPlayerSummon()))
				harmful = false;
