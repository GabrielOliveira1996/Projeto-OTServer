...
Monster::Monster(MonsterType *_mType) : Creature()
{
	...
	if (mType)
	{
		...
		this->Creature::nameDescription = mType->nameDescription; // puxa a descri��o
	}
	spawn = NULL;
	raid = NULL;
...
bool Monster::isTarget(Creature *creature)
{
	...
	// Verificação de Storage para Players
	if (Player *targetPlayer = creature->getPlayer())
	{
		// Usamos a fun��o global para checar se este jogador tem permiss�o
		// para interagir com este monstro (this)
		if (!canPlayerAttackMonster(targetPlayer, this))
		{
			// Se o player n�o tem a storage necess�ria para este monstro,
			// o monstro ignora o player (n�o ataca e n�o persegue)
			return false;
		}
	}
	return true;
...
void Monster::doAttacking(uint32_t interval)
{
	...
	/**
	 * bônus de velocidade de ataque do akamaru.
	 */
	uint32_t agilityBonus = 0;

	// verifica se o monstro é um akamaru
	if (isAkamaru && master)
	{
	...
	for (SpellList::iterator it = mType->spellAttackList.begin(); it != mType->spellAttackList.end(); ++it)
	{
		...
		if (canUseSpell(myPos, targetPos, *it, attackTicks, inRange))
		{
			if (it->chance >= (uint32_t)random_range(1, 100))
			{
				...
				/**
				 * bônus de ataque do akamaru.
				 */
				if (isAkamaru && master) // verifica se de fato é um akamaru
				{
					Player *playerMaster = master->getPlayer();
					if (playerMaster)
					{
						...
						/**
						 * é subtraido para que o bônus de ataque
						 * funcione mesmo se o monstro tiver um
						 * valor de ataque negativo, por exemplo:
						 * (minCombatValue = -50, maxCombatValue = -20)
						 * */
...
bool Monster::getNextStep(Direction &dir, uint32_t &flags)
{
	...
	if (target)
	{
		...
		if (targetPlayer)
		{
			// Se o monstro for um dos "Mestres" e o player n�o tiver a storage
			if (!canPlayerAttackMonster(targetPlayer, this))
			{
	...
	{
		// Se n�o houver alvo e o monstro for um dos protegidos,
		// podemos impedir o movimento aleat�rio (opcional, igual voc� fez no Mizuki)
		// Usamos uma verifica��o simples: o monstro est� na lista?
...
{
	if (damage > 0 && isAkamaru && isSummon() && master)
	{
		Player *playerMaster = master->getPlayer();
		if (playerMaster)
		{
...
bool Monster::convinceCreature(Creature *creature)
{
	...
	// [CORREÇÃO]: A verificação agora é restrita apenas ao monstro chamado Akamaru.
	// Isso impede que NPCs de escolta como o Tazuna herdem os status do pet.
	if (getName() == "Akamaru")
	{
		this->isAkamaru = true;
		...
		if (playerMaster)
		{
			// Verifica se o mestre é um Inuzuka para aplicar os pontos de HP/Speed
			...
			if (vocId >= 33 && vocId <= 37)
			{
				...
				// akamaru speed movement
				uint32_t newSpeed = playerMaster->getAkamaruMaxSpeed();

				// variation reset to 0 to prevent speed accumulates
				this->varSpeed = 0;

				// reset speed with the new speed
				this->baseSpeed = newSpeed;

				// use changespeed to set the new speed e força o update no cliente
	...
	// Se o mestre (creature) estiver atacando alguém, o monstro ataca o mesmo alvo.
	if (Creature *masterTarget = creature->getAttackedCreature())
	{
		if (masterTarget != this && masterTarget != creature)
		{
			...
			// Como estamos dentro da classe Monster, acessamos 'idle' diretamente
			// Isso tira o Bunshin do estado de paralisia imediatamente.
			this->isIdle = false;
		}
	}
	// ------------------------------------
