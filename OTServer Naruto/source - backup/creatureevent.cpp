...
CreatureEvents::CreatureEvents() : m_interface("CreatureScript Interface")
{
...
std::string CreatureEvent::getScriptEventName() const
{
	...
	case CREATURE_EVENT_EXTENDED_OPCODE: // Adicione isso
...
uint32_t CreatureEvent::executeExtendedOpcode(Player *player, uint8_t opcode, const std::string &buffer)
{
	if (m_interface->reserveEnv())
	{
		...
		if (m_scripted == EVENT_SCRIPT_BUFFER)
		{
			std::stringstream scriptstream;
			scriptstream << "local cid = " << env->addThing(player) << std::endl;
			scriptstream << "local opcode = " << (uint32_t)opcode << std::endl;
		...
		else
		{
			...
			lua_pushnumber(L, env->addThing(player));
			lua_pushnumber(L, (uint32_t)opcode);
