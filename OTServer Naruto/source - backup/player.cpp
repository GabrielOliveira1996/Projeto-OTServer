...
void Player::addAkamaruPoint(const std::string &attr)
{
	...
	if (akamaruPoints == 0)
	{
		sendTextMessage(MSG_STATUS_SMALL, "You have no Akamaru points available.");
		return;
	}
	...
	{
		akamaruPoints--;
		sendTextMessage(MSG_STATUS_SMALL, "Point added successfully!");
		this->sendAkamaruData();
	}
	else if (limitReached)
	{
		sendTextMessage(MSG_STATUS_SMALL, "This attribute has already reached the maximum level (100).");
	}
}
...
void Player::addExperience(uint64_t exp)
{
	...
	if (exp > 0)
	{
		...
		{
			uint64_t akamaruBonus = (uint64_t)(exp * 0.70);
			addAkamaruExperience(akamaruBonus);
		}
	}
	...
	while (experience >= nextLevelExp)
	{
		...
		// Adiciona os pontos na memória do servidor
		this->addAttributePoints(1);
	...
	if (prevLevel != level)
	{
		...
		uint32_t gainedPoints = (level - prevLevel) * 1; // Ajustado para 3 conforme o seu loop acima
		char advMsg[60];
		sprintf(advMsg, "You advanced from Level %d to Level %d and received %d attribute points.", prevLevel, level, gainedPoints);
		sendTextMessage(MSG_EVENT_ADVANCE, advMsg);

		// --- LINHA DEFINITIVA ADICIONADA ---
		// Envia o pacote 0x34 para o OTClient atualizar a label de pontos e mostrar os botões +
		if (client)
		{
