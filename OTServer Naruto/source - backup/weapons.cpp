...
Weapon::Weapon(LuaScriptInterface *_interface) : Event(_interface)
{
	id = 0;
	level = 0;
	magLevel = 0;
...
void Weapon::onUsedWeapon(Player *player, Item *item, Tile *destTile) const
{
	...
	if (manaCost > 0)
	{
		...
		if (!player->hasFlag(PlayerFlag_NotGainMana) && (player->getZone() != ZONE_PVP || !g_config.getBool(ConfigManager::PVPZONE_ADDMANASPENT)))
...
WeaponMelee::WeaponMelee(LuaScriptInterface *_interface) : Weapon(_interface)
{
...
int32_t WeaponMelee::getWeaponDamage(const Player *player, const Creature *target, const Item *item, bool maxDamage /*= false*/) const
{
	...
	/**
	 * Sistema de Jutsus da Hinata.
	 * Byakugan, Juken e Lion Fist.
	 **/
	uint32_t vocId = player->getVocationId();
	if (vocId >= 53 && vocId <= 56)
	{
		if (!item || item->getWeaponType() == WEAPON_FIST)
		{
			std::string sJuken, sBya, sLion;
			bool hasJuken = const_cast<Player *>(player)->getStorage(45003, sJuken) && atoi(sJuken.c_str()) > 0;
			bool hasByakugan = const_cast<Player *>(player)->getStorage(45001, sBya) && atoi(sBya.c_str()) > 0;
			bool hasLion = const_cast<Player *>(player)->getStorage(45005, sLion) && atoi(sLion.c_str()) > 0;

			int32_t level = player->getLevel();
			int32_t chanceBase = 20 + (level / 5); // aumenta 1% a cada 5 níveis.
			if (chanceBase > 100)
				chanceBase = 100; // limite de 100%

			double damageMultiplier = 1.0;
			bool activatedJuken = false;
			bool activatedLion = false;

			// Juken, ganha mais 5% de chance.
			if (hasJuken && random_range(1, 100) <= (chanceBase + 5))
			{
				damageMultiplier += 0.25; // +25%
				activatedJuken = true;
			}

			// Byakugan, ganha +10% de chance.
			if (hasByakugan && random_range(1, 100) <= (chanceBase + 10))
			{
				damageMultiplier += 0.50; // +50%
			}

			// Lion Fist, ganha +15% de chance.
			if (hasLion && random_range(1, 100) <= (chanceBase + 15))
			{
				damageMultiplier += 1.25; // +125% (Dano x2.25)
				activatedLion = true;
			}

			// soma total dos multiplicadores ao dano 'ret'
			ret = (int32_t)(ret * damageMultiplier);

			// lógica de dreno e efeitos visuais
			if (target && (activatedJuken || activatedLion))
			{
				int32_t finalDamage = (maxDamage ? ret : random_range(0, ret, DISTRO_NORMAL));
				// dreno acumulativo: Juken (50%) + Lion (50%) = 100% se ambos ativarem
				...
				int32_t manaDrain = (int32_t)(finalDamage * drainPercent);

				if (manaDrain > 0)
				{
					const_cast<Creature *>(target)->changeMana(-manaDrain);
					char manaText[20];
					sprintf(manaText, "-%d", manaDrain);
					g_game.addAnimatedText(target->getPosition(), 35, manaText);
				}

				// prioridade de efeito: se Lion ativou no hit, efeito 56. Senão, 24.
				uint16_t effectId = (activatedLion ? 56 : 24);
...
WeaponDistance::WeaponDistance(LuaScriptInterface *_interface) : Weapon(_interface)
{
	hitChance = -1;
...
WeaponWand::WeaponWand(LuaScriptInterface *_interface) : Weapon(_interface)
{
	minChange = 0;
	maxChange = 0;
