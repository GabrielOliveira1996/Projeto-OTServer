	...
	for (CountMap::const_iterator it = damageMap.begin(); it != damageMap.end(); ++it)
	{
		...
		for (DeathList::iterator fit = list.begin(); fit != list.end(); ++fit)
		{
			...
			if (!(mdc->getName() == tmp->getName() && mdc->getMaster() == tmp->getMaster()) &&
				(!mdc->getMaster() || (mdc->getMaster() != tmp && mdc->getMaster() != tmp->getMaster())) && (mdc->getSummonCount() <= 0 || tmp->getMaster() != mdc))
				continue;
...
void Creature::changeHealth(int32_t healthChange)
{
	if (healthChange < 0)
	{
		// Sistema de Esquiva do Akamaru (Mantido)
		if (getName() == "Akamaru" && isSummon() && master)
		{
			...
			if (playerMaster)
			{
				...
				uint32_t dodgeChance = std::min((uint32_t)80, (uint32_t)(dodgePts / 3));
		...
		// REMOVEMOS A TRAVA DE SAGA DAQUI!
		// Ela já é checada no Combat::canDoCombat.
		// Aqui dentro, ela causa o bug do "lastHitCreature" ser NULL no primeiro hit.
	}
...
bool Creature::hasCondition(ConditionType_t type, int32_t subId /* = 0*/, bool checkTime /* = true*/) const
{
	...
	for (ConditionList::const_iterator it = conditions.begin(); it != conditions.end(); ++it)
	{
		...
		if (!checkTime || g_config.getBool(ConfigManager::OLD_CONDITION_ACCURACY) || !(*it)->getEndTime() || (*it)->getEndTime() >= OTSYS_TIME())
			return true;
	}
...
/*void Creature::setName(const std::string& newName)
{
	...
	if(id != 0)
	{
		...
		// ou o m�todo de Disappear/Appear que remove os "fantasmas"
