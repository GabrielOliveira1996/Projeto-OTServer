	...
	if (creature && target)
	{
		...
		if (player && summon)
		{
			...
			else
			{
				...
				// Na sua source a propriedade � defaultOutfit (confirmado em erros anteriores)
			...
			// ATUALIZA��O DE NOME E VISUAL (Protocolo Antigo)
			for (AutoList<Player>::iterator it = Player::autoList.begin(); it != Player::autoList.end(); ++it)
			{
				if (it->second->canSeeCreature(summon))
				{
					// O erro na imagem 9 confirmou que a fun��o pede (const Creature*, Visible_t)
...
int32_t LuaScriptInterface::luaDoSendPlayerExtendedOpcode(lua_State *L)
{
	...
	// A corre��o est� aqui: acessamos o client/protocolo do player
	if (player->client)
	{
...
int32_t LuaScriptInterface::luaDoPlayerRemoveMana(lua_State *L)
{
	...
	// metodo oficial de remo��o de mana do player.
