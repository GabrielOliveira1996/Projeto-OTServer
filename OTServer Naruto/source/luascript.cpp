...
void LuaScriptInterface::registerFunctions()
{
	...
	//CreateBunshin(name, pos[, displayError = true], playerName)
	lua_register(m_luaState, "luaCreateBunshin", LuaScriptInterface::luaCreateBunshin);
...
int32_t LuaScriptInterface::luaCreateBunshin(lua_State* L)
{
	...
	std::string newName = "";
	if (params >= 3) newName = popString(L);
	...
	if (!newName.empty()) {
		...
		monster->setNameDescription("a " + newName);
	}
