...
int32_t Player::getDefense() const
{
    ...
    // 3. Verificação de segurança para a Vocação (Evita o Crash)
    if(vocation)
    {
    ...
    // 4. Cálculo final com cast explícito para evitar erros de precisão
...
void Player::drainMana(Creature* attacker, CombatType_t combatType, int32_t damage)
{
	...
	char buffer[150];
	if(attacker)
		sprintf(buffer, "You lose %d ki blocking an attack by %s.", damage, attacker->getNameDescription().c_str());
	else
		sprintf(buffer, "You lose %d ki.", damage);
...
void Player::addManaSpent(uint64_t amount, bool useMultiplier/* = true*/)
{
	...
	while(manaSpent + amount >= nextReqMana)
	{
		...
		char advMsg[50];
		sprintf(advMsg, "Your ki power has been increased to %d.", magLevel);
