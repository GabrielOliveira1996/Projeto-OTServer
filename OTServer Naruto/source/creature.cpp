...
void Creature::changeHealth(int32_t healthChange)
{
	if(healthChange < 0)
    {
        // Importante: Declarar a função externa para o compilador encontrá-la
        extern bool canPlayerAttackMonster(const Player* player, const Creature* target);
        // 'this' é o monstro recebendo o dano (Mizuki, Zabuza, etc.)
        ...
        if(attacker)
        {
            ...
            if(attackerPlayer)
            {
                // Verifica dinamicamente se o player pode causar dano a ESTE monstro
        ...
        {
            // Se não há um atacante direto (ex: dano de field/fogo)
            // verificamos se este monstro (this) está na lista de protegidos
            extern std::map<std::string, uint32_t> monsterProtections;
            if(monsterProtections.find(getName()) != monsterProtections.end())
            {
                // Bloqueia danos de fontes sem dono para monstros da lista
                return;
            }
        }
    }

	// Código original para aplicar a mudança (agora possivelmente zerada)
	if(healthChange > 0)
...
{
    // Declaramos a função externa para o compilador
    extern bool canPlayerAttackMonster(const Player* player, const Creature* target);
    // Definimos quem é o atacante (seja o passado por parâmetro ou o último a bater)
    ...
    if(targetAttacker) {
        Player* attackerPlayer = targetAttacker->getPlayer();
        if(attackerPlayer) {
            // 'this' é o monstro sofrendo o dano
            if(!canPlayerAttackMonster(attackerPlayer, this)) {
                g_game.addMagicEffect(getPosition(), MAGIC_EFFECT_POFF);
    ...
    // Se não há atacante direto, verificamos se o monstro atual está na lista de protegidos
    else {
        extern std::map<std::string, uint32_t> monsterProtections;
        if(monsterProtections.find(getName()) != monsterProtections.end()) {
...
/*const std::string& Creature::getNameDescription() const
{
...
/*void Creature::setName(const std::string& newName)
{
    ...
    if(id != 0)
    {
        ...
        // Tente usar o internalChange se sua base suportar, 
        // ou o método de Disappear/Appear que remove os "fantasmas"
        g_game.internalCreatureChangeOutfit(this, currentOutfit);
    }
}*/

/*void Creature::setNameDescription(const std::string& newDescription)
{
