...
void loadProtections() {
    ...
    monsterProtections["Shukako Two"] = 11142;
...
{
    // Buscamos a criatura que está no topo do tile selecionado
    const Creature* target = const_cast<Tile*>(tile)->getTopCreature();
    if(caster && target)
    {
        if(!canPlayerAttackMonster(caster->getPlayer(), target))
        {
            // Bloqueia o início do ataque (seleção do quadrado vermelho ou uso de magia de área)
