...
int32_t Player::getDefense() const
{
	...
	// 3. Verifica��o de seguran�a para a Voca��o (Evita o Crash)
	if (vocation)
	{
	...
	// 4. C�lculo final com cast expl�cito para evitar erros de precis�o
...
/*void AkamaruData::addExperience(uint64_t gain, Player *master)
{
	if (gain <= 0 || !master)
		return;
	exp += gain;

	while (exp >= getRequiredExp(level + 1))
	{
		level++;
		points += 3; // Ganha 3 pontos para distribuir por nível

		char buf[128];
		sprintf(buf, "Seu Akamaru subiu para o level %d!", level);
		master->sendTextMessage(MSG_EVENT_ADVANCE, buf);

		extern Game g_game;
		g_game.addMagicEffect(master->getPosition(), (MagicEffect_t)10); // Efeito de fumaça/brilho
	}
}*/

// --- Métodos de Ponte da Classe Player ---

/*void Player::addAkamaruExperience(uint64_t exp)
{
	if (akamaruData)
	{
		akamaruData->addExperience(exp, this);
	}
}
...
uint32_t Player::getAkamaruMaxHealth() const
{
	if (!akamaruData)
		return 0;
	return 150 + (akamaruData->healthPts * 10); // Vida base 150 + 10 por ponto
}
...
uint32_t Player::getAkamaruMaxSpeed() const
{
	if (!akamaruData)
		return 0;
	return 200 + (akamaruData->speedPts * 5); // Velocidade base 200 + 5 por ponto
}
...
uint64_t Player::getAkamaruRequiredExpToNextLevel() const
{
	if (!akamaruData)
		return 0;
	return akamaruData->getRequiredExp(akamaruData->level + 1);
}*/

// formula para experiencia necessaria
/*uint64_t AkamaruData::getRequiredExp(uint32_t lvl) const
{
	if (lvl <= 1)
		return 0;
	return (uint64_t)((50ULL * lvl * lvl * lvl - 150ULL * lvl * lvl + 400ULL * lvl) / 3ULL) - 100ULL;
}*/
...
void Player::addExperience(uint64_t exp)
{
	...
	/*if (exp > 0)
	{
		const std::list<Creature *> &summonsList = getSummons();
		for (std::list<Creature *>::const_iterator it = summonsList.begin(); it != summonsList.end(); ++it)
		{
			Creature *summon = *it;
			if (dynamic_cast<Akamaru *>(summon))
			{
				if (akamaruData)
				{
					uint64_t akamaruBonus = (uint64_t)(exp * 0.70);
					akamaruData->addExperience(akamaruBonus, this);
				}
				break;
			}
		}
...
bool Player::onDeath()
{
	...
	if (skillLoss)
	{
		...
		for (int16_t i = 0; i < 7; ++i) // for each skill
		{
			...
			for (uint32_t c = 2; c <= skills[i][SKILL_LEVEL]; ++c) // sum up all required tries for all skill levels
