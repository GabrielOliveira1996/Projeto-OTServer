...
{
	// trava da saga para dano em area
	if (target && target->getMonster())
	{
		if (SagaMonster *sagaMonster = dynamic_cast<SagaMonster *>(target->getMonster()))
		{
			if (caster && caster->getPlayer())
			{
				if (!sagaMonster->canBeAttackedBy(static_cast<Player *>(caster)))
				{
					return;
				}
			}
