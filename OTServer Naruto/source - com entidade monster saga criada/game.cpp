	...
	if (g_config.getBool(ConfigManager::STORE_TRASH))
	{
		...
		if (g_config.getBool(ConfigManager::CLEAN_PROTECTED_ZONES))
		{
			for (; it != trash.end(); ++it)
			{
				...
				{
					if ((*tit)->isMoveable() && !(*tit)->isLoadedFromMap() && !(*tit)->isScriptProtected())
					{
							  ...
							  const Position &movingCreaturePos, const Position &toPos)
{
	...
	if (!player->hasCustomFlag(PlayerCustomFlag_CanThrowAnywhere) && ((std::abs(pos.x - toPos.x) > movingCreature->getThrowRange()) || (std::abs(pos.y - toPos.y) > movingCreature->getThrowRange()) || (std::abs(pos.z - toPos.z) * 4 > movingCreature->getThrowRange())))
	{
	...
	if (player != movingCreature)
	{
		...
		if ((movingCreature->getZone() == ZONE_PROTECTION || movingCreature->getZone() == ZONE_NOPVP) && !toTile->hasFlag(TILESTATE_NOPVPZONE) && !toTile->hasFlag(TILESTATE_PROTECTIONZONE) && !player->hasFlag(PlayerFlag_IgnoreProtectionZone))
		{
			player->sendCancelMessage(RET_NOTPOSSIBLE);
			return false;
		}

		if (toTile->getCreatures() && !toTile->getCreatures()->empty() && !player->hasFlag(PlayerFlag_CanPushAllCreatures))
		{
			player->sendCancelMessage(RET_NOTPOSSIBLE);
			return false;
		}
...
ReturnValue Game::internalMoveCreature(Creature *creature, Direction direction, uint32_t flags /* = 0*/)
{
	...
	if (direction < SOUTHWEST && creature->getPlayer())
	{
		...
		{
			if ((!(tmpTile = map->getTile(Position(currentPos.x, currentPos.y, currentPos.z - 1))) || (!tmpTile->ground && !tmpTile->hasProperty(BLOCKSOLID))) &&
				(tmpTile = map->getTile(Position(destPos.x, destPos.y, destPos.z - 1))) && tmpTile->ground && !tmpTile->hasProperty(BLOCKSOLID))
			{
	...
	if (!index)
	{
		...
		if (player->hasCustomFlag(PlayerCustomFlag_CanSeeItemDetails))
		{
			...
			if (it.transformEquipTo)
				ss << std::endl
				   << "TransformTo (onEquip): [" << it.transformEquipTo << "]";
			else if (it.transformDeEquipTo)
				ss << std::endl
				   << "TransformTo (onDeEquip): [" << it.transformDeEquipTo << "]";
		...
		for (it = container->getItems(); it != container->getEnd(); ++it)
		{
			...
			if (player->hasCustomFlag(PlayerCustomFlag_CanSeeItemDetails))
			{
				...
				if (iit.transformEquipTo)
					...
					ss << std::endl
					   << "TransformTo: [" << iit.transformDeEquipTo << "] (onDeEquip).";
...
bool Game::internalCloseTrade(Player *player)
{
	...
	{
		std::cout << "[Warning - Game::internalCloseTrade] TradeState == TRADE_TRANSFER, " << player->getName() << " " << player->getTradeState() << ", " << tradePartner->getName() << " " << tradePartner->getTradeState() << std::endl;
		return true;
	}
...
bool Game::playerLookAt(uint32_t playerId, const Position &pos, uint16_t spriteId, int16_t stackpos)
{
	...
	if (player->hasCustomFlag(PlayerCustomFlag_CanSeeCreatureDetails))
	{
		if (const Creature *creature = thing->getCreature())
		{
			...
			{
				ss << std::endl
				   << "IP: " << convertIPAddress(destPlayer->getIP()) << ", Client: " << destPlayer->getClientVersion() << ".";
	...
	if (player->hasCustomFlag(PlayerCustomFlag_CanSeePosition))
		ss << std::endl
		   << "Position: [X: " << thingPos.x << "] [Y: " << thingPos.y << "] [Z: " << thingPos.z << "].";
...
bool Game::playerSetAttackedCreature(uint32_t playerId, uint32_t creatureId)
{
	...
	if (Monster *monster = attackCreature->getMonster())
	{
		if (SagaMonster *sagaMonster = dynamic_cast<SagaMonster *>(monster))
		{
			if (!sagaMonster->canBeAttackedBy(player))
			{
				player->sendCancelMessage(RET_YOUMAYNOTATTACKTHISCREATURE);
				player->sendCancelTarget();
				player->setAttackedCreature(NULL);
				return false;
			}
