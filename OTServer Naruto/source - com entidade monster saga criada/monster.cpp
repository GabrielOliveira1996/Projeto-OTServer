...
{
	if (SagaMonster::isSagaMonsterName(mType->name))
	{
		return new SagaMonster(mType);
	}
...
void Monster::doAttacking(uint32_t interval)
{
	...
	if (getName() == "Akamaru" && isSummon() && master)
	{
		...
		if (playerMaster) //&& playerMaster->getAkamaruData()
		{
			// Cada ponto reduz 10ms do intervalo (Limite de 1700ms)
			uint32_t agility = playerMaster->getAkamaruData()->agility;
			agilityBonus = std::min((uint32_t)1700, (uint32_t)(agility * 10));

			// Garantimos que o intervalo resultante nao seja menor que 300ms
		...
		if (canUseSpell(myPos, targetPos, *it, interval, inRange))
		{
			if (it->chance >= (uint32_t)random_range(1, 100))
			{
				...
				if (getName() == "Akamaru" && isSummon() && master)
				{
					...
					if (playerMaster) //&& playerMaster->getAkamaruData()
					{
						float attackPts = (float)playerMaster->getAkamaruData()->attack;
