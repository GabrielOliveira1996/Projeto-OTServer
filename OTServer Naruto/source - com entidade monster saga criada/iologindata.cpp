...
bool IOLoginData::loadPlayer(Player *player, const std::string &name, bool preLoad /*= false*/)
{
	...
	player->id = result->getDataInt("id");
	...
	if (akamaruResult)
	{
		AkamaruData *data = new AkamaruData();
		data->level = akamaruResult->getDataInt("level");
		data->exp = akamaruResult->getDataLong("exp");
		data->points = akamaruResult->getDataInt("points");
		data->attack = akamaruResult->getDataInt("attack");
		data->agility = akamaruResult->getDataInt("agility");
		data->dodge = akamaruResult->getDataInt("dodge");
		data->healthPts = akamaruResult->getDataInt("health_pts");
		data->speedPts = akamaruResult->getDataInt("speed_pts");

		player->setAkamaruData(data);
		akamaruResult->free();
	}
	else
	{
		player->setAkamaruData(new AkamaruData());
	}*/
...
bool IOLoginData::savePlayer(Player *player, bool preSave /* = true*/, bool shallow /* = false*/)
{
	...
	// Salvamento do Sistema Akamaru
	/*if (AkamaruData *data = player->getAkamaruData())
	{
		std::stringstream akamaruQuery;
		akamaruQuery << "REPLACE INTO `player_akamarus` (`player_id`, `level`, `exp`, `points`, `attack`, `agility`, `dodge`, `health_pts`, `speed_pts`) VALUES ("
					 << player->getGUID() << ", "
					 << data->level << ", "
					 << data->exp << ", "
					 << data->points << ", "
		...
		if (!db->executeQuery(akamaruQuery.str()))
		{
			std::clog << "[Error - IOLoginData::savePlayer] Erro ao salvar dados do Akamaru para: " << player->getName() << std::endl;
		}
	}*/
