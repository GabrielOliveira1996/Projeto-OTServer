local config = {
    chance_quebrar = 60,       -- 60% de chance de sumir ao usar
    efeito_tiro = CONST_SEBON_THROW,
    efeito_impacto = 0,        
    cooldown = 1,              
...
function onUse(cid, item, fromPosition, itemEx, toPosition)
    ...
    if exhaustion.check(cid, config.storage_exhaust) then
        doPlayerSendCancel(cid, "Aguarde " .. exhaustion.get(cid, config.storage_exhaust) .. " segundo(s) para usar novamente.")
        return true
    end
    ...
    local min = 15 + (skill * 1.2) + (level * 0.2)
    local max = 25 + (skill * 1.8) + (level * 0.4)
