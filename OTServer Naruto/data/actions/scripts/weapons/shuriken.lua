local config = {
    chance_quebrar = 45, -- 45% de chance de sumir ao usar
    efeito_tiro = CONST_SHURIKEN_THROW, 
    efeito_impacto = 0,
    cooldown = 1,
...
function onUse(cid, item, fromPosition, itemEx, toPosition)
    ...
    if exhaustion.check(cid, config.storage_exhaust) then
        doPlayerSendCancel(cid, "Aguarde " .. exhaustion.get(cid, config.storage_exhaust) .. " segundo(s) para usar a shuriken novamente.")
        return true
    end
    ...
    local min = 10 + (skill * 1.3) + (level * 0.2)
    local max = 20 + (skill * 1.9) + (level * 0.4)
