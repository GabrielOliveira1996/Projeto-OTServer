...
local vocation = 68 -- Vocation ID
local mana = 150 -- Mana necessária
local outfit = {lookType = 100, lookHead = 0, lookBody = 0, lookLegs = 0, lookFeet = 0, lookTypeEx = 0, lookAddons = 0} -- Outfit
local useable_vocs = {"Neji Train", "Neji", "Byakugan Power"} -- Vocations que podem usar a talk
local level = 80
local levelMax = 150

if not isInArray(useable_vocs, getPlayerVocationName(cid)) then
doPlayerSendCancel(cid, "Essa vocaçao nao possui o byakugan.") return true
...
setPlayerStorageValue(cid, 8157, getPlayerVocation(cid))
doSetCreatureOutfit(cid, outfit, -1)
doPlayerSetVocation(cid, vocation)
addEvent(doPlayerSetVocation, tempo*1000, cid, getPlayerStorageValue(cid, 8157))
