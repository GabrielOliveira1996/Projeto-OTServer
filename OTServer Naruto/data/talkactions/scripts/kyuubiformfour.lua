...
local vocation = 67 -- Vocation ID
local mana = 400 -- Mana necessária
local outfit = {lookType = 357, lookHead = 0, lookBody = 0, lookLegs = 0, lookFeet = 0, lookTypeEx = 0, lookAddons = 0} -- Outfit
local useable_vocs = {"Naruto Shippuden", "Kyuubi Form Four"} -- Vocations que podem usar a talk
local level = 160
local levelMax = 300
...
setPlayerStorageValue(cid, 8156, getPlayerVocation(cid))
doSetCreatureOutfit(cid, outfit, -1)
doPlayerSetVocation(cid, vocation)
addEvent(doPlayerSetVocation, tempo*1000, cid, getPlayerStorageValue(cid, 8156))
