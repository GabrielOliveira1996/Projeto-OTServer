...
function init()
  ...
  consolePanel = g_ui.loadUI('console', gameRootPanel)
  ...
  consoleTabBar = consolePanel:getChildById('consoleTabBarPanel')
  sayModeButton = consolePanel:getChildById('sayModeButton')
  
  prevChannelButton = consolePanel:getChildById('prevChannelButton')
  nextChannelButton = consolePanel:getChildById('nextChannelButton')
  closeChannelButton = consolePanel:getChildById('closeChannelButton')

  -- Configuração das Abas (TabBar)
  if consoleTabBar then
    consoleTabBar:setContentWidget(consoleContentPanel)
    consoleTabBar.onTabChange = onTabChange
    consoleTabBar.blinkTab = function() end
  end

  -- Configuração do Input de Texto (LOGICA DE NPC INJETADA AQUI)
    ...
    consoleTextEdit.onKeyPress = function(self, keyCode, keyboardModifiers)
      if keyCode == KeyEnter then
        ...
        if #message > 0 then
          ...
          -- SE FOR ABA DE NPC, ENVIA COMO PRIVATE (A "BOSTA" QUE FALTAVA)
          if currentTab and currentTab.npcChat then
            -- Usamos talkPrivate para NPCs com o modo de fala correto (NpcTo = 12 ou 14)
            -- name é o nome do NPC que está no texto da aba
            g_game.talkPrivate(MessageModes.NpcTo, currentTab:getText(), message)
            
            -- Adiciona o texto visualmente na aba pra você não achar que não enviou
            local player = g_game.getLocalPlayer()
            local prefix = applyMessagePrefixies(g_game.getCharacterName(), player:getLevel(), message)
            addPrivateText(prefix, SpeakTypesSettings.privatePlayerToNpc, currentTab:getText(), false, g_game.getCharacterName())
          
          else
            -- SE NÃO FOR NPC, SEGUE O SEU CÓDIGO ORIGINAL QUE FUNCIONA NO DEFAULT
            local channelId = (currentTab and currentTab.channelId) or 0
            g_game.talkChannel(MessageModes.Say, channelId, message)
          end
  ...
  -- Atalhos de Teclado
  g_keyboard.bindKeyDown('Ctrl+O', g_game.requestChannelList, gameRootPanel)
  g_keyboard.bindKeyDown('Ctrl+Tab', function() consoleTabBar:selectNextTab() end, gameRootPanel)
  g_keyboard.bindKeyDown('Ctrl+Shift+Tab', function() consoleTabBar:selectPrevTab() end, gameRootPanel)
  g_keyboard.bindKeyDown('Enter', function() toggleChat() end, gameRootPanel)
...
function onTextMessage(mode, text)
  -- LISTA NEGRA: Modos que NÃƒO devem ir para o console/chat de jeito nenhum
  local ignoredModes = {
    [MessageModes.Failure] = true, -- "You are full", "Hungry"
    [MessageModes.Status]  = true, -- Status em geral
    [MessageModes.Game]    = true  -- Mensagens genÃ©ricas de evento
  }

  -- Se o modo estiver na lista negra, apenas mandamos para a TELA e encerramos
  if ignoredModes[mode] then
    modules.game_textmessage.displayMessage(mode, text)
    return 
  end

  -- REGISTRO NORMAL (Dano, Loot, etc)
  local serverLogTab = consoleTabBar:getTab(tr('Registro do servidor'))
  if serverLogTab then
    addTabText(text, SpeakTypesSettings.channelWhite, serverLogTab)
  else 
    addText(text, SpeakTypesSettings.channelWhite, tr('Default'))
  end
end
...
function toggleChat()
  if consoleTextEdit:isVisible() and consoleTextEdit:isFocused() then
    return -- Não faz nada, deixa o onKeyPress cuidar do envio
  end
...
function enableChat()
  ...
  -- Atualiza o botão para o estado "digitando" (Branco/W)
  if modules.game_interface and modules.game_interface.updateWasdButton then
    modules.game_interface.updateWasdButton()
  end
...
function disableChat()
  ...
  -- 1. Primeiro devolvemos o foco para o jogo
  local root = modules.game_interface.getRootPanel()
  if root then
    root:focus()
  end

  -- 2. Depois ligamos o módulo de andar
  if modules.game_walking then 
    modules.game_walking.enableWSAD() 
  end

  -- 3. Por fim, atualizamos o botão para verde
  if modules.game_interface and modules.game_interface.updateWasdButton then
    modules.game_interface.updateWasdButton()
  end
end
...
function isChatEnabled()
  return not consoleToggleChat:isChecked() and consoleTextEdit:isEnabled()
end

function terminate()
  -- 1. SALVAR CONFIGURAï¿½ï¿½ES
  save()
  saveCommunicationSettings()

  -- 2. DESCONECTAR EVENTOS DO JOGO
  disconnect(g_game, {
    onTalk = onTalk,
    onChannelList = onChannelList,
    onOpenChannel = onOpenChannel,
  ...
  -- 3. LIMPAR DADOS SE ESTIVER ONLINE
  if g_game.isOnline() then 
    clear() 
  end

  -- 4. REMOVER ATALHOS DE TECLADO (IMPORTANTï¿½SSIMO)
  local gameRootPanel = modules.game_interface.getRootPanel()
  ...
  -- 5. DESTRUIR JANELAS AUXILIARES (SE EXISTIREM)
  ...
  -- 6. LIMPAR REFERï¿½NCIAS DOS COMPONENTES
  consoleTabBar = nil
  consoleContentPanel = nil
  consoleToggleChat = nil
  consoleTextEdit = nil
  ...
  -- 7. DESTRUIR O PAINEL PRINCIPAL (O QUE CRIAMOS NO INIT)
  if consolePanel then
    consolePanel:destroy()
    consolePanel = nil
  end

  -- Limpa a classe global do mï¿½dulo
  Console = nil
end
...
function clear()
  -- Se nÃ£o houver canais ou o console jÃ¡ estiver limpo, saÃ­mos para evitar erros
  ...
  -- PROTEÃ‡ÃƒO: Verifica se channels Ã© uma tabela antes do pairs
  ...
  -- close channels
  -- PROTEÃ‡ÃƒO: Verifica se channels e consoleTabBar existem
  if type(channels) == 'table' and consoleTabBar then
  ...
  -- PROTEÃ‡ÃƒO: SÃ³ remove as abas fixas se elas existirem no consoleTabBar
  if consoleTabBar then
    if defaultTab then
...
function sendMessage(message, tab)
  ...
  if modules.game_interface.isWasdEnabled() then
      modules.game_interface.toggleWasd()
  end
end
