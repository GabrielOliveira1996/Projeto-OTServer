...
function init()
  ...
  if consoleTabBar then
    ...
    -- Mata o erro de blinkTab que trava o console
    consoleTabBar.blinkTab = function() end
  end

  -- Configuração do Input de Texto (Foco e Digitação)
  if consoleTextEdit then
    ...
    consoleTextEdit.onKeyPress = function(self, keyCode, keyboardModifiers)
      if keyCode == KeyEnter then
        ...
        if #message > 0 then
          -- Pega o modo de fala (Say, Whisper, etc)
          local mode = sayModeButton:getData().sayMode
          if SpeakTypesSettings and SpeakTypesSettings[mode] then
            local speakType = SpeakTypesSettings[mode]
            local currentTab = consoleTabBar:getSelectedTab()
            if currentTab then
              local channelId = currentTab.channelId
              if speakType.private then
                g_game.talkPrivate(speakType.speakType, self.receiver, message)
              else
                g_game.talkChannel(speakType.speakType, channelId, message)
              end
            end
          end
          self:clearText()
      ...
      return false -- Permite que as letras apareçam na caixa
    end
  end

  -- Conexões de Eventos do Jogo (Crucial para as abas funcionarem)
  connect(g_game, {
    onTalk = onTalk,
  ...
  -- O bind do Enter que ativa o chat (Padrão que você usa)
...
function enableChat()
  ...
  -- Força o destravamento do mouse de qualquer outro widget
  g_mouse.ungrabMouse()

  addEvent(function()
    consoleTextEdit:setEnabled(true)
    consoleTextEdit:setFocusable(true)
    consoleTextEdit:raise() -- Garante que ele está na frente de todos os painéis
