...
function updateSpelllist()
  ...
  local playerVoc = localPlayer:getVocation()
  for i = 1, #SpelllistSettings[SpelllistProfile].spellOrder do
    local spell = SpelllistSettings[SpelllistProfile].spellOrder[i]
    local info = SpellInfo[SpelllistProfile][spell]
    local tmpLabel = spellList:getChildById(spell)
    ...
    if tmpLabel then
      -- REMOVEMOS os filtros complexos de Knight/Sorcerer
      -- E verificamos apenas se a vocação do player está na lista da magia
      if table.find(info.vocations, playerVoc) then
...
function updateSpelllist()
  ...
  -- Pega a vocação atual do personagem (Ex: 37)
  local playerVoc = localPlayer:getVocation()
  ...
  for i = 1, #SpelllistSettings[SpelllistProfile].spellOrder do
    ...
    if tmpLabel then
      -- Verifica se a vocação do player está na lista de vocações permitidas da magia
      -- No NTO, as magias costumam ter uma tabela como: vocations = {37, 38, 39}
      if table.find(info.vocations, playerVoc) then
        tmpLabel:setVisible(true)
      else
        -- Se você quiser que o filtro manual ainda funcione por cima disso, 
        -- pode adicionar as outras checagens aqui. Caso contrário, apenas oculte:
        tmpLabel:setVisible(false)
      end
    end
  end
