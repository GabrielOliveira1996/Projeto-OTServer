...
function EnterGame.loginManager()
  g_platform.openUrl("http://seusiteaqui.com/register")
end
...
function EnterGame.init()
  ...
  if not enterGame then
    print("ERRO: entergame.otui não foi carregado.")
    return
  end

  -- Buscar widgets de forma segura
  rememberPasswordBox = enterGame:recursiveGetChildById('rememberPasswordBox')
  ...
  if not accountTextEdit then
    print("ERRO: accountNameTextEdit não encontrado no .otui")
  end

  if not passwordTextEdit then
    print("ERRO: accountPasswordTextEdit não encontrado no .otui")
  end

  if not rememberPasswordBox then
    print("ERRO: rememberPasswordBox não encontrado no .otui")
  end

  -- Carregar dados salvos (com proteção contra nil)
  local savedAccount = g_settings.get('account')
  local savedPassword = g_settings.get('password')

  local account = savedAccount and g_crypt.decrypt(savedAccount) or ""
  local password = savedPassword and g_crypt.decrypt(savedPassword) or ""

  -- Preencher campos somente se existirem
  if accountTextEdit then
    accountTextEdit:setText(account or "")
  end

  if passwordTextEdit then
    passwordTextEdit:setText(password or "")
  end

  if rememberPasswordBox then
    rememberPasswordBox:setChecked(account and #account > 0)
  end
...
function EnterGame.clearAccountFields()
  local acc = enterGame:getChildById('accountNameTextEdit')
  local pass = enterGame:getChildById('accountPasswordTextEdit')

  if acc then acc:clearText() end
  if pass then pass:clearText() end
...
function EnterGame.doLogin(account, password, token, host)
  ...
  local server_ip = "26.238.18.62" 
  local server_port = 7171      
  local server_version = 854
  ...
  G.account = account or enterGame:recursiveGetChildById('accountNameTextEdit'):getText()
  G.password = password or enterGame:recursiveGetChildById('accountPasswordTextEdit'):getText()
  G.authenticatorToken = token or ""
  G.stayLogged = true
  G.server = "Local"
  G.host = host or "26.238.18.62:7171"
  G.clientVersion = 854
