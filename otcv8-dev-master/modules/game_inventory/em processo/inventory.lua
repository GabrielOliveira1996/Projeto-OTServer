...
function init()
  ...
  if not inventoryWindow then
    print("ERRO CRITICO: Nao foi possivel carregar o arquivo inventory.otui!")
    return
  end

  inventoryWindow:disableResize()
  
  -- Referência ao seu painel customizado (Naruto + Slots)
  inventoryPanel = inventoryWindow:recursiveGetChildById('customInventoryPanel')

  if not inventoryPanel then
    print("ERRO: O customInventoryPanel nao foi encontrado no OTUI!")
  end
  
  -- Limpeza: Esconde qualquer widget antigo que esteja sobrando no contentsPanel
  ...
  if contentsPanel then
    for _, child in pairs(contentsPanel:getChildren()) do
      -- Se for o nosso painel novo, GARANTE que ele apareça
      if child:getId() == 'customInventoryPanel' then
        child:show()
        child:setPhantom(true) -- Garante que o painel de fundo não bloqueie cliques
      elseif child:getId() ~= 'customInventoryPanel' then
        -- Aqui você esconde os widgets originais do Tibia que sobraram
        child:hide()
      end
    end
  end
  ...
  -- Controls (Se eles não estiverem no seu novo OTUI, esses comandos apenas não farão nada)
  ...
  if fightOffensiveBox then fightModeRadioGroup:addWidget(fightOffensiveBox) end
  if fightBalancedBox then fightModeRadioGroup:addWidget(fightBalancedBox) end
  if fightDefensiveBox then fightModeRadioGroup:addWidget(fightDefensiveBox) end

  connect(fightModeRadioGroup, { onSelectionChange = onSetFightMode })
  
  if chaseModeButton then connect(chaseModeButton, { onCheckChange = onSetChaseMode }) end
  if safeFightButton then connect(safeFightButton, { onCheckChange = onSetSafeFight }) end
  
  connect(LocalPlayer, { onVocationChange = updateVocationFigure })
...
function terminate()
  ...
  disconnect(LocalPlayer, { onStatesChange = onStatesChange,
                         ...
                         onFreeCapacityChange = onFreeCapacityChange })
  -- status end
  disconnect(LocalPlayer, { onVocationChange = updateVocationFigure })
...
function toggleAdventurerStyle(hasBlessing)
  for slot = InventorySlotFirst, InventorySlotLast do
    local widgetId = InventorySlotStyles[slot]
    if widgetId then
      local itemWidget = inventoryWindow:recursiveGetChildById(widgetId)
      if itemWidget then
        -- Use setChecked ou setOn em vez de mudar cores de fundo para evitar o erro de nil value
...
function refresh()
  ...
  print("Debug: Funcao refresh() executada")
  if not inventoryWindow then return end

  -- Atualiza os slots um por um
  ...
  -- Atualiza o Naruto e Status
  if player then
    onSoulChange(player, player:getSoul())
    onFreeCapacityChange(player, player:getFreeCapacity())
    onStatesChange(player, player:getStates(), 0)
    updateVocationFigure() -- Garante que o Naruto apareça
  end

  local hasAdventurer = player and Bit.hasBit(player:getBlessings(), Blessings.Adventurer) or false
  toggleAdventurerStyle(hasAdventurer)
...
function onInventoryChange(player, slot, item, oldItem)
  -- PROTEÇÃO MESTRA: Se a janela ainda não existe no carregamento, ignora e não crasha
  if not inventoryWindow then 
    return 
  end
  ...
  if itemWidget then
    ...
    -- Print para sabermos se o item chegou no slot certo
    if item then
       print("Debug Terminal: Slot " .. widgetId .. " recebeu item ID " .. item:getId())
    else
       print("Debug Terminal: Slot " .. widgetId .. " esta vazio.")
    end
  else
    -- Se aparecer isso, o ID que você colocou no OTUI não bate com a tabela InventorySlotStyles
    print("ERRO Terminal: O codigo tentou achar o '" .. widgetId .. "' mas ele nao existe no OTUI!")
  end
end

function updateVocationFigure()
    local player = g_game.getLocalPlayer()
    if not player or not inventoryPanel then return end
    
    local figureWidget = inventoryPanel:getChildById('vocationFigure')
    if not figureWidget then return end

    local vocationId = player:getVocation()
    -- Caminho onde você vai salvar as imagens (ex: 1.png, 2.png)
    local imagePath = '/images/vocations/' .. vocationId
    
    if g_resources.fileExists(imagePath .. '.png') then
        figureWidget:setImageSource(imagePath)
    else
        -- Caso não tenha a imagem da vocação, usa uma padrão ou esconde
        figureWidget:setImageSource('/images/vocations/1')
    end
end
