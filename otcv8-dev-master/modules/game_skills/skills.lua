...
function init()
  ...
  connect(g_game, {
    onGameStart = refresh,
    onGameEnd = offline,
    onAttributePointsChange = onAttributePointsChange
  })
  ...
  -- Primeiro carregamos a UI
  skillsWindow = g_ui.loadUI('skills', modules.game_interface.getRightPanel())
  
  -- Segundo: O SETUP deve vir antes do REFRESH
  skillsWindow:setup()
  
  -- Terceiro: Ativamos o botão do menu
  skillsButton:setOn(true)
  
  -- Por fim, se já estiver online, atualizamos os dados
...
function onAttributePointsChange(points)
  
  if not skillsWindow then return end

  local label = skillsWindow:recursiveGetChildById('attributePointsLabel')
  if label then
    label:setText(tr("Points: %d", points))
    label:setColor(points > 0 and '#00ff00' or '#ff0000')
  end

  -- Atualiza visibilidade dos botões
  local skillIds = {'magiclevel', 'skillId0', 'skillId1', 'skillId2', 'skillId3', 'skillId4', 'skillId5', 'skillId6'}
  for _, id in pairs(skillIds) do
    local widget = skillsWindow:recursiveGetChildById(id)
    if widget then
      local btn = widget:getChildById('addButton')
      if btn then btn:setVisible(points > 0) end
    end
  end
end

function onAddPoint(id)
  local protocol = g_game.getProtocolGame()
  if protocol then
    -- Envia para o servidor qual skill queremos upar
    -- O 'id' será 'skillId0', 'magiclevel', etc, conforme definido no OTUI
    protocol:sendAddAttributePoint(id)
  else
    print("Erro: Protocolo de jogo não encontrado.")
  end
end
