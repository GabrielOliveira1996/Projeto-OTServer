...
function init()
  ...
  -- 4. CONECTAR EVENTOS (Somente após identificar os widgets)
  ...
  addEvent(function()
    if gameMapPanel then
      -- Chamamos apenas uma vez aqui fora da updateSize
...
function refreshViewMode()
  ...
  -- Em vez de fill(parent), deixamos o updateSize cuidar da dimensão
...
function updateSize()
  ...
  -- Se a janela estiver minimizada, evitamos erro de divisão por zero
  ...
  -- 1. ZOOM (ALTURA): Mantemos 15 para o zoom padrão que você gosta (Perto).
  local viewHeight = 15 
  
  -- 2. CÁLCULO DA LARGURA: 
  local viewWidth = math.floor(viewHeight * ratio)

  -- 3. GARANTIR ÍMPAR (Personagem no centro):
  if viewWidth % 2 == 0 then viewWidth = viewWidth + 1 end

  -- 4. LIMITES DE SEGURANÇA (O teto do seu C++ e Servidor):
  -- Use o maior valor que você configurou (ex: 43 ou 45)
  local maxViewportX = 43 
  
  if viewWidth > maxViewportX then viewWidth = maxViewportX end
  if viewWidth < 15 then viewWidth = 15 end 

  -- 5. TRAPAÇA MATEMÁTICA (O SEGREDO DO ULTRATHIN BLACK BARS):
  -- Como o client desenha em blocos inteiros (32px), se a janela não for perfeita,
  -- sobrarão alguns pixels pretos nas bordas. 
  -- Para forçar a imagem a cobrir esses últimos pixels SEM achatar o sprite,
  -- nós aumentamos a largura em +2 SQMs e deixamos o KeepAspectRatio ativo.
  -- O client vai desenhar os SQMs extras "atrás" das UI laterais, preenchendo o preto.
  ...
  -- REATIVAR TRUE: Isso garante que o sprite do personagem Fique PERFEITO e sem distorção.
