...
function init()
  ...
  gameRootPanel:hide()
  gameRootPanel:lower()
  gameRootPanel:setFocusable(true) -- ForÁa o root a aceitar foco
  
  -- 2. IDENTIFICAR O MAPA
  gameMapPanel = gameRootPanel:getChildById('gameMapPanel')
  gameMapPanel:setFocusable(true) -- ForÁa o mapa a aceitar foco
  
  -- 3. IDENTIFICAR OS PAIN…IS
  ...
  -- ESTA FUN«√O … O SEGREDO: Se o foco for para o "nada", ele volta para o mapa
  rootWidget.onFocusChange = function(self, focused)
    if not focused and gameMapPanel then
      addEvent(function() gameMapPanel:focus() end)
    end
  end

  -- Detecta fechamento da janela de contagem
  connect(rootWidget, { 
    onChildHide = function(widget, child)
      if child:getId() == 'countWindow' or child:getStyleName() == 'CountWindow' then
        addEvent(function()
...
local wasdEnabled = false

function toggleWasd()
  local console = modules.game_console
  if not console then return end

  -- Em vez de tentar chamar toggleChat (que pode ser local), 
  -- chamamos as funÁıes que vocÍ j· definiu como globais
  if console.consoleTextEdit:isVisible() then
    console.disableChat()
  else
    console.enableChat()
  end
...
function updateWasdButton()
  local button = rootWidget:recursiveGetChildById('wasdControlButton')
  local console = modules.game_console
  if not button or not console then return end

  -- Se o chat n esta visivel, o WASD esta ativo
  if not console.consoleTextEdit:isVisible() then
    button:setText("WASD")
    button:setColor("#00ff00") -- Verde
  else
    button:setText("W")
    button:setColor("#ffffff") -- Branco
  end
end
...
function onGameStart()
  g_game.changeMapAwareRange(27, 21)
  refreshViewMode()
  show()
...
function updateStretchShrink()
  if modules.client_options.getOption('dontStretchShrink') and not alternativeView then
    ...
    local heightDiff = (gameMapPanel:getHeight() - 32 * 15)
    bottomSplitter:setMarginBottom(bottomSplitter:getMarginBottom() + heightDiff - 10)
  end
end
...
function moveStackableItem(item, toPos)
  ...
  local moveFunc = function()
    ...
    local window = okButton:getParent()
    window:destroy()
    countWindow = nil
    
    -- ISSO SIMULA UM "SOLTAR DE MOUSE" GLOBAL
    rootWidget:ungrabMouse() 
    modules.game_interface.getRootPanel():focus()
    if gameMapPanel then gameMapPanel:focus() end
  end
  local cancelButton = countWindow:getChildById('buttonCancel')
  local cancelFunc = function()
    local window = cancelButton:getParent()
    window:destroy()
    countWindow = nil
    
    -- Aplica a mesma soluÁ„o que funcionou no OK
    rootWidget:ungrabMouse() 
...
function refreshViewMode()
  if not g_game.isOnline() or not gameMapPanel then return end

  -- Em vez de fill(parent), deixamos o updateSize cuidar da dimens√£o
...
function updateSize()
  if updatingSize or not gameRootPanel or not gameMapPanel then return end
  updatingSize = true 

  local rect = gameMapPanel:getRect()
  
  if rect.height <= 0 or rect.width <= 0 then 
    updatingSize = false
    return 
  end
  ...
  local viewHeight = 15 
  local viewWidth = math.floor(viewHeight * ratio)

  if viewWidth % 2 == 0 then viewWidth = viewWidth + 1 end

  local maxViewportX = 43 
  if viewWidth > maxViewportX then viewWidth = maxViewportX end
  if viewWidth < 15 then viewWidth = 15 end 

  local finalViewWidth = viewWidth + 2
  if finalViewWidth > maxViewportX + 2 then finalViewWidth = maxViewportX + 2 end
  
  gameMapPanel:setVisibleDimension({width = finalViewWidth, height = viewHeight})
  gameMapPanel:setKeepAspectRatio(true) 

  -- Reposicionamento ForÁado das Action Bars ao mudar tamanho da tela
  addEvent(function()
    if gameRootPanel then
      -- Percorre as possÌveis barras (ajuste o 10 se tiver mais barras)
      for i = 1, 10 do
        local bar = gameRootPanel:getChildById('actionBarPanel' .. i)
        if bar then
          bar:breakAnchors()
          bar:addAnchor(AnchorBottom, 'parent', AnchorBottom)
          bar:addAnchor(AnchorHorizontalCenter, 'parent', AnchorHorizontalCenter)
