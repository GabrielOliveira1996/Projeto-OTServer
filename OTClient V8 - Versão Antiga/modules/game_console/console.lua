...
function init()
  ...
  -- 1. FIX DE ERROS GLOBAIS (Obrigatório para options.lua)
  _G.consoleToggleChat = consolePanel:getChildById('toggleChat') or g_ui.createWidget('UIWidget', consolePanel)
  ...
  if consoleTabBar and consoleContentPanel then
    consoleTabBar:setContentWidget(consoleContentPanel)
    consoleTabBar.blinkTab = function() end 
  end

  -- 2. SISTEMA DE ENVIO DO CHAT (onKeyDown restaurado)
  if consoleTextEdit then
    consoleTextEdit.onKeyDown = function(self, keyCode)
      if keyCode == KeyEnter then
        ...
        if #message > 0 then
          if sendMessage then 
            sendMessage(message, consoleTabBar:getCurrentTab()) 
          end
          self:clearText()
        end
        return true 
  ...
  -- 3. FUNÇÃO DE REPOSICIONAMENTO DA ACTIONBAR
  local function forceShowActionBars()
    local root = modules.game_interface.getRootPanel()
    local bar = root:recursiveGetChildById("actionBar") or root:recursiveGetChildById("actionbar.1")
    
    if bar then
      bar:setParent(root)
      bar:setVisible(true)
      bar:setImageSource('')
      bar:setBackgroundColor('alpha')
      
      -- 1. CENTRALIZAÇÃO NA TELA
      bar:removeAnchor(AnchorBottom)
      bar:removeAnchor(AnchorLeft)
      bar:removeAnchor(AnchorHorizontalCenter) -- Limpa se houver anterior

      -- Fixa no fundo (Bottom) com 45px de margem (mesma altura que estava)
      bar:addAnchor(AnchorBottom, 'parent', AnchorBottom)
      bar:setMarginBottom(45)

      -- Fixa no CENTRO HORIZONTAL da tela toda
      bar:addAnchor(AnchorHorizontalCenter, 'parent', AnchorHorizontalCenter)
      
      -- Largura fixa para os 12 slots (12 * 36px)
      bar:setWidth(440) 
      bar:setHeight(60)

      -- 2. ORGANIZAÇÃO DOS SLOTS (Garantindo que fiquem dentro da barra centralizada)
      local tabBar = bar:getChildById('tabBar')
      if tabBar then
        tabBar:fill('parent')
        local slots = tabBar:getChildren()
        local startX = 0
        for i, slot in ipairs(slots) do
          if slot:getId():find('slot') then
            slot:setVisible(true)
            slot:removeAnchor(AnchorLeft)
            slot:setSize({width = 34, height = 50})
            slot:setMarginLeft(startX)
            
            -- Lógica visual do Hotkey Display
            local label = slot:getChildById('hotkeyLabel')
            local display = slot:getChildById('hotkeyDisplay')
            if label and display then
              display:setVisible(label:getText() ~= "")
            end
  ...
  -- Aumentamos a frequência para 100ms para "ganhar" do script original
  cycleEvent(function() pcall(forceShowActionBars) end, 100)
...
function terminate()
  ...
  -- 4. REMOVER ATALHOS DE TECLADO (IMPORTANTÍSSIMO)
  local gameRootPanel = modules.game_interface.getRootPanel()
  ...
  -- 6. LIMPAR REFERÊNCIAS DOS COMPONENTES
  consoleTabBar = nil
  consoleContentPanel = nil
  consoleToggleChat = nil
  consoleTextEdit = nil
  ...
  -- Limpa a classe global do módulo
  Console = nil
end
